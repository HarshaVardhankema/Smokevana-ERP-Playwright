...
use App\Business;
use App\Jobs\WooCommerceWebhookPipeline;
use App\Transaction;
...
class WoocommerceWebhookController extends Controller
{
    ...
    /**
     * Function to receive webhook from WooCommerce
     * 
     * @param  Request  $request
     * @param  int  $business_id
     * @return void
     */
    public function receiveWebhook(Request $request, $business_id)
    {
        $business = Business::findOrFail($business_id);
        $payload = $request->getContent();
        $is_valid_request = $this->isValidWebhookRequest($request, $business->woocommerce_wh_general_secret);
        if ($is_valid_request) {
            ...
            switch ($event) {
                case 'product_created':
                    WooCommerceWebhookPipeline::dispatch($business_id, $data, $business->woocommerce_api_settings);
                    break;
                case 'product_updated':
                    WooCommerceWebhookPipeline::dispatch($business_id, $data, $business->woocommerce_api_settings);
                    break;
                case 'product_stock_updated':
                    $this->woocommerceUtil->processWooCommerceProductStockUpdate($business_id, $data, $business->woocommerce_api_settings);
                    break;
                case 'product_price_updated':
                    $this->woocommerceUtil->processWooCommerceProductPriceUpdate($business_id, $data, $business->woocommerce_api_settings);
                    break;
                case 'product_status_updated':
                    $this->woocommerceUtil->processWooCommerceProductStatusUpdate($business_id, $data, $business->woocommerce_api_settings);
                    break;
                case 'order_created':
                        // Log::info('Order created hits');
                        $user_id = $business->owner->id;
                        $woocommerce_api_settings = $this->woocommerceUtil->get_api_settings($business_id);
                        $business_data = [
                            'id' => $business_id,
                            ...
                            // Log::info('order data',['data'=>$data]);
                       // Convert array to object for compatibility with existing methods
                       $order_object = json_decode(json_encode($data));
                       $this->woocommerceUtil->createNewSaleFromOrder($business_id, $user_id, $order_object, $business_data);
                       break;
                default:
