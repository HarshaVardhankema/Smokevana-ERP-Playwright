...
class B2BPaymentOrderHelper
{
    /**
     * Get products with B2B pricing (price_group_prices instead of sell_price_inc_tax)
     */
    public static function getB2BProductsWithRelations($productIds, $userId, $priceGroupId, $locationId, $isKeyByID = false)
    {
        $products = Product::with([
            ...
            'variations' => function ($query) use ($locationId, $priceGroupId) {
                $query->select([
                    'variations.id',
                    'variations.name',
    ...
    /**
     * Process cart items for B2B customer orders - NOW WITH FREE ITEMS SUPPORT
     */
    public static function processCartItemsForB2B($cartItems, $products, $transaction, $userId, $currentTime, $cartUtil, $discountService, $discounts, $appliedDiscounts, $taxCharges, $userState, &$appliedDiscountBucket)
    {
        $sellLinesData = [];
        ...
        foreach ($cartItems as $cartItem) {
            ...
            if ($product) {
                ...
                $appliedDiscount = $cartUtil->calculateHighPriorityDiscount($appliedDiscounts, $discounts, $product, $variation, $cartItem, $discountService);
                ...
                $sellLinesData[] = [
                    ...
                    'is_free' => false,
                    // Keep column set consistent with other sell lines (e.g. gift cards)
                    'sell_line_note' => null,
                    'created_at' => now(),
                    'updated_at' => now(),
                ];
        ...
        if (!empty($freeItemsToAdd)) {
            ...
            // Get contact info for price group
            $contact = Contact::find($userId);
            $priceTier = $contact ? $contact->price_tier : [];
            $priceGroupId = key($priceTier);
            
            $freeProducts = self::getB2BProductsWithRelations(
                $freeProductIds, 
                $userId, 
                $priceGroupId, 
            ...
            foreach ($freeItemsToAdd as $item) {
                ...
                foreach ($variationsToCheck as $variation) {
                    ...
                    // Get price for free item display
                    $priceToDisplay = 0;
                    if ($variation->group_prices && $variation->group_prices->isNotEmpty()) {
                        $priceToDisplay = $variation->group_prices->first()->price_inc_tax ?? 0;
                    }
                    ...
                    $sellLinesData[] = [
                        ...
                        'unit_price_before_discount' => $priceToDisplay,
                        'unit_price_inc_tax' => 0, // Free item
                        'item_tax' => 0,
                        'line_discount_type' => 'fixed',
                        'line_discount_amount' => $priceToDisplay, // Full discount
                        'is_free' => true,
                        // Keep column set consistent with other sell lines (e.g. gift cards)
                        'sell_line_note' => null,
                        'created_at' => now(),
    ...
    /**
     * Calculate final total for B2B customer orders
     * Returns array with total and shipping details matching Cart API logic
     */
    public static function calculateFinalTotal($cartItems, $products, $cartUtil, $discountService, $discounts, $appliedDiscounts, $taxCharges, $userState, $shipping_charges, $shippingType, $userId)
    {
        $cart_final_total = 0;
        $cartDiscountApplicable = true;
        $isFreeShippingApplicable = true;
    ...
    /**
     * Calculate total before tax for B2B customer orders
     */
    ...
    /**
     * Process payment for B2B customer orders using existing PaymentOrderController
     */
    public static function processPayment($nonce, $payedAmount, $newValue, $userId, $cart, $transaction, $paymentOrderController)
    {
        ...
        if (config('app.gatewayType') == 'authorizenet') {
            ...
            if ($payByAuthorize['status'] === true) {
                Log::info('Authorize B2B Payment Success');
                return ['status' => true, 'transaction_id' => $payByAuthorize['transaction_id']];
            } else {
                return ['status' => false, 'message' => $payByAuthorize['message']];
            }
        } else {
            $saleData = $paymentOrderController->doSale($payedAmount, $nonce, $billingInfo, $shippingInfo);
