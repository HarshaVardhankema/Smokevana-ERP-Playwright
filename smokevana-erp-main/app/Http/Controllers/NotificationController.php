...
class NotificationController extends Controller
{
    ...
    public function getTemplate($id, $template_for)
    {
        ...
        elseif ($template_for == 'registration_approve_confirmation') {
            $contact = Contact::find($id);
        }
        elseif ($template_for == 'registration_reject_confirmation') {
            $contact = Contact::find($id);
        }
        elseif ($template_for == 'contact_us_send') {
    ...
    public function getTemplatePayment(Request $request)
    {
        $contact = Contact::where('business_id',request()->session()->get('business.id'))->find(request()->input('contact_id'));
        // prepare request for payment
        if(!$contact){
            return response()->json(['status'=>false,'message'=>'No contact in this business']);
        }

        $token =Str::random(40);
        $data =[];
        $data = $request->only(['contact_id', 'due_payment_type', 'method', 'paid_on', 'amount', 'contact_type_radio', 'transactions', 'card_number', 'card_holder_name', 'card_transaction_number', 'card_type', 'card_month', 'card_year', 'card_security', 'cheque_number', 'bank_account_number', 'transaction_no_1', 'transaction_no_2', 'transaction_no_3', 'note']);
       
        $payment = PaymentBuffer::updateOrCreate([
          'business_id' => request()->session()->get('business.id'),
          'location_id' =>1,
          'customer_id' => $request->input('contact_id'),
          'payment_method' => 'custom_payment_1',
          ],
  [
          'payment_ref_no' => null,
          'token' => $token,
        ...
        $data = PaymentBuffer::with('contact')->where('id',$payment->id)->first();
        // return $data;
        $transaction = $data;
    ...
    public function send(Request $request)
    {
        ...
        try {
            ...
            }else if($request->input('template_for') == 'send_payment_notification'){
                $paymentData = $request->input('paymentData');
               // If it's JSON encoded on the frontend (as we're doing with JSON.stringify)
               $paymentData = json_decode($paymentData, true); 

                $tag_replaced_data = $this->notificationUtil->replaceCustomTagsPayments($business_id, $orig_data, $paymentData);

                $data['email_body'] = $tag_replaced_data['email_body'];
                $data['sms_body'] = $tag_replaced_data['sms_body'];
                $data['subject'] = $tag_replaced_data['subject'];
                $data['whatsapp_text'] = $tag_replaced_data['whatsapp_text'];
            }else if ($request->input('template_for') == 'registration_approve_confirmation') {
                $contact_id = $request->input('contact_id');
                $contact = Contact::find(id: $contact_id);
                $tag_replaced_data = $this->notificationUtil->replaceCustomTags($business_id, $orig_data, $contact);

                $data['email_body'] = $tag_replaced_data['email_body'];
                $data['sms_body'] = $tag_replaced_data['sms_body'];
                $data['subject'] = $tag_replaced_data['subject'];
                $data['whatsapp_text'] = $tag_replaced_data['whatsapp_text'];
            }else if ($request->input('template_for') == 'registration_reject_confirmation') {
                $contact_id = $request->input('contact_id');
                $contact = Contact::find(id: $contact_id);
                $tag_replaced_data = $this->notificationUtil->replaceCustomTags($business_id, $orig_data, $contact);
                ...
                $data['subject'] = $tag_replaced_data['subject'];
                $data['whatsapp_text'] = $tag_replaced_data['whatsapp_text'];
            }else if ($request->input('template_for') == 'contact_us_send') {
                // For contact_us_send, transaction_id is actually the ContactUs id
                $contact_us = ContactUs::find($transaction_id);
                if ($contact_us) {
                    $tag_replaced_data = $this->notificationUtil->replaceCustomTags($business_id, $orig_data, $contact_us);
            ...
            elseif (array_key_exists($request->input('template_for'), $generalNotifications)) {
                if (in_array('email', $notification_type)) {
