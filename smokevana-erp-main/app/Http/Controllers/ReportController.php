...
class ReportController extends Controller
{
    ...
    /**
     * Get profit/loss chart data for graph visualization
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getProfitLossChartData(Request $request)
    {
        if (! auth()->user()->can('profit_loss_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        // Get profit/loss data grouped by date
        ...
        $query->select(
            DB::raw('DATE(sale.transaction_date) as date'),
        ...
        foreach ($results as $result) {
            $labels[] = \Carbon\Carbon::parse($result->date)->format('M d');
            $profitData[] = (float) $result->gross_profit;
        }
    ...
    /**
     * Get purchase and sale chart data for graph visualization
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getPurchaseSellChartData(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $purchaseQuery = Transaction::where('business_id', $business_id)
            ->where('type', 'purchase')
            ->where('status', 'received');

        if ($permitted_locations != 'all') {
            $purchaseQuery->whereIn('location_id', $permitted_locations);
        }

        if (! empty($location_id)) {
            $purchaseQuery->where('location_id', $location_id);
        }

        if (! empty($start_date) && ! empty($end_date)) {
            $purchaseQuery->whereDate('transaction_date', '>=', $start_date)
        ...
        // Get sale data grouped by date
        $saleQuery = Transaction::where('business_id', $business_id)
            ->where('type', 'sell')
            ->where('status', 'final');

        if ($permitted_locations != 'all') {
            $saleQuery->whereIn('location_id', $permitted_locations);
        }

        if (! empty($location_id)) {
            $saleQuery->where('location_id', $location_id);
        }

        if (! empty($start_date) && ! empty($end_date)) {
            $saleQuery->whereDate('transaction_date', '>=', $start_date)
                  ->whereDate('transaction_date', '<=', $end_date);
        }
        ...
        // Combine dates and create labels
        $allDates = collect($purchaseData->pluck('date'))
            ->merge($saleData->pluck('date'))
            ->unique()
            ->sort()
            ->values();
        ...
        $purchaseMap = $purchaseData->pluck('total', 'date')->toArray();
        $saleMap = $saleData->pluck('total', 'date')->toArray();

        foreach ($allDates as $date) {
            $labels[] = \Carbon\Carbon::parse($date)->format('M d');
            $purchaseValues[] = isset($purchaseMap[$date]) ? (float) $purchaseMap[$date] : 0;
            $saleValues[] = isset($saleMap[$date]) ? (float) $saleMap[$date] : 0;
        }
        ...
        if ($request->ajax()) {
            $contacts = Contact::where('contacts.business_id', $business_id)
                ...
                ->select(
                    ...
                    // erp_custom
                    // Opening balance due value (without sign adjustments)
                    DB::raw("(SUM(IF(t.type = 'opening_balance', final_total, 0)) - SUM(IF(t.type = 'opening_balance', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0))) as opening_balance_due_value"),
                    // Computed due value for ordering
                    DB::raw("(
                        (SUM(IF(t.type = 'sell' AND t.status = 'final', final_total, 0))
                         - SUM(IF(t.type = 'sell' AND t.status = 'final', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0))
                         - SUM(IF(t.type = 'ledger_discount' AND sub_type='sell_discount', final_total, 0)))
                        -
                        (SUM(IF(t.type = 'purchase', final_total, 0))
                         - SUM(IF(t.type = 'purchase', (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0))
                         - SUM(IF(t.type = 'ledger_discount' AND sub_type='purchase_discount', final_total, 0)))
                        -
                        (SUM(IF(t.type = 'sell_return', final_total, 0))
                         - SUM(IF(t.type = 'sell_return', (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)))
                        +
                        (SUM(IF(t.type = 'purchase_return', final_total, 0))
                         - SUM(IF(t.type = 'purchase_return', (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)))
                        +
                        CASE WHEN contacts.type = 'supplier'
                             THEN -1 * (SUM(IF(t.type = 'opening_balance', final_total, 0))
                                        - SUM(IF(t.type = 'opening_balance', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)))
                             ELSE      (SUM(IF(t.type = 'opening_balance', final_total, 0))
                                        - SUM(IF(t.type = 'opening_balance', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)))
                        END
                    ) as due_value"),
                    //  erp_custom
    ...
    /**
     * Get customer supplier report chart data for graph visualization
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getCustomerSupplierChartData(Request $request)
    {
        if (! auth()->user()->can('contacts_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $start_date = ! empty(request()->input('start_date')) ? request()->input('start_date') : $fy['start'];
        $end_date = ! empty(request()->input('end_date')) ? request()->input('end_date') : $fy['end'];
        $contact_type = ! empty(request()->input('contact_type')) ? request()->input('contact_type') : null;

        $permitted_locations = auth()->user()->permitted_locations();
        
        // Get purchase and sale totals by date
        $query = Transaction::where('transactions.business_id', $business_id)
            ->whereIn('transactions.type', ['purchase', 'sell'])
            ->where(function($q) {
                $q->where(function($q2) {
                    $q2->where('transactions.type', 'purchase')->where('transactions.status', 'received');
                })->orWhere(function($q2) {
                    $q2->where('transactions.type', 'sell')->where('transactions.status', 'final');
                });
            })
            ->leftjoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ->select(
                DB::raw('DATE(transactions.transaction_date) as date'),
                DB::raw('SUM(IF(transactions.type="purchase", transactions.final_total, 0)) as total_purchase'),
                DB::raw('SUM(IF(transactions.type="sell", transactions.final_total, 0)) as total_sell')
            );
        ...
        if (! empty($contact_type)) {
            $query->whereIn('contacts.type', [$contact_type, 'both']);
        }
        ...
        $results = $query->groupBy(DB::raw('DATE(transactions.transaction_date)'))
            ->orderBy('date', 'asc')
            ->get();
        ...
        foreach ($results as $result) {
            $labels[] = \Carbon\Carbon::parse($result->date)->format('M d');
            $purchaseValues[] = (float) $result->total_purchase;
            $sellValues[] = (float) $result->total_sell;
        }
    ...
    public function getStockReport(Request $request)
    {
        ...
        $is_group_pricing_enabled = $this->moduleUtil->isModuleEnabled('group_pricing');
        foreach ($selling_price_groups as $selling_price_group) {
            if (auth()->user()->can('selling_price_group.'.$selling_price_group->id)&& $is_group_pricing_enabled) {
        ...
        if ($request->ajax()) {
            ...
            $products = $this->productUtil->getProductStockDetailsModelPopup($business_id, $filters, $for);
            ...
            if ($for == 'view_product' && ! empty(request()->input('product_id'))) {
                ...
                // Get price groups with sequence for sorting
                $price_groups = \App\SellingPriceGroup::getSortedBySequence($business_id);
                $price_group_name_to_sequence = [];
                ...
                if ($business && !empty($business->common_settings)) {
                    ...
                    if (!empty($common_settings['price_group_sequence'])) {
                        $price_group_sequence = $common_settings['price_group_sequence'];
                    }
                }
                
                // Create mapping from price group name to sequence
                foreach ($price_groups as $pg) {
                    $sequence = isset($price_group_sequence[$pg->id]) ? $price_group_sequence[$pg->id] : 9999;
                    $price_group_name_to_sequence[$pg->name] = $sequence;
                }

                return view('product.partials.product_stock_details')->with(compact('product_stock_details', 'has_b2b_access', 'price_group_name_to_sequence'));
            }
            ...
            $datatable = Datatables::of($products)
                ...
                ->editColumn('webstock', function ($row) {
                    if ($row->enable_stock) {
                        $webstock = $row->webstock ? $row->webstock : 0;

                        return  '<span class="current_stock" data-orig-value="'.(float) $webstock.'" data-unit="'.$row->unit.'"> '.$this->transactionUtil->num_f($webstock, false, null, true).'</span>'.' '.$row->unit;
                    } else {
                        return '--';
                    }
                })
                ...
                ->editColumn('unit_price', function ($row) use ($allowed_selling_price_group) {
                    ...
                    if ($allowed_selling_price_group) {
                        $html .= ' <button type="button" class="tw-dw-btn tw-dw-btn-xs text-white  tw-dw-btn-primary tw-w-max btn-modal no-print" data-container=".view_modal" data-href="'.action([\App\Http\Controllers\ProductController::class, 'viewGroupPrice'], [$row->product_id]).'">'.__('lang_v1.view_group_prices').'</button>';
                    }
            ...
            $raw_columns = ['unit_price', 'total_transfered', 'total_sold',
                'total_adjusted', 'stock','webstock' ,'stock_price', 'stock_value_by_sale_price',
    ...
    /**
     * Shows gift card stock report
     *
     * @return \Illuminate\Http\Response
     */
    public function getGiftCardStockReport(Request $request)
    {
        if (! auth()->user()->can('product.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            $filters = request()->only(['location_id', 'category_id', 'sub_category_id', 'brand_id']);

            // Get gift cards with stock information
            $query = \App\Product::where('business_id', $business_id)
                ->where('is_gift_card', 1)
                ->where('productVisibility', 'public')
                ->where('enable_selling', 1)
                ->where('is_inactive', 0)
                ->with(['webcategories', 'brand']);
            ...
            // Format gift card data for datatables
            $gift_cards_data = [];
            foreach ($gift_cards as $gift_card) {
                $gift_cards_data[] = [
                    ...
                    'name' => $gift_card->name,
                    'sku' => $gift_card->sku,
                    'category' => $gift_card->webcategories->first()->name ?? '',
                    'brand' => $gift_card->brand->name ?? '',
                    'price_range' => $gift_card->price_range,
                    'gift_card_stock' => $gift_card->gift_card_stock,
                    'gift_card_expiry_days' => $gift_card->gift_card_expiry_days,
                    'allow_partial_redemption' => $gift_card->allow_partial_redemption ?? true,
                    'created_at' => $gift_card->created_at->format('Y-m-d H:i:s'),
                    'action' => '<button class="btn btn-xs btn-primary view-gift-card" data-id="' . $gift_card->id . '">View</button>'
                ];
            }
        ...
        return view('report.gift_card_stock_report')
            ->with(compact('categories', 'brands', 'business_locations'));
    }

    /**
     * Get gift card details for view modal
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getGiftCardDetails(Request $request)
    {
        if (! auth()->user()->can('product.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $gift_card = \App\Product::where('business_id', $business_id)
            ->where('is_gift_card', 1)
            ->where('id', $gift_card_id)
            ->with(['webcategories', 'brand', 'product_locations'])
            ->first();
        ...
        // Get gift card usage statistics
        $sold_gift_cards = \App\GiftCard::where('product_id', $gift_card_id)
            ->where('status', 'sold')
            ->count();

        $active_gift_cards = \App\GiftCard::where('product_id', $gift_card_id)
            ->where('status', 'active')
            ->count();

        $expired_gift_cards = \App\GiftCard::where('product_id', $gift_card_id)
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'short_description' => $gift_card->short_description ?? '',
                'image_url' => $gift_card->image_url ?? '',
                'price_range' => $gift_card->price_range,
                'gift_card_stock' => $gift_card->gift_card_stock,
                'gift_card_expiry_days' => $gift_card->gift_card_expiry_days,
                'allow_partial_redemption' => $gift_card->allow_partial_redemption ?? true,
                ...
                'brand' => $gift_card->brand->name ?? '',
                'created_at' => $gift_card->created_at->format('Y-m-d H:i:s'),
                'updated_at' => $gift_card->updated_at->format('Y-m-d H:i:s'),
                'statistics' => [
                    'sold_count' => $sold_gift_cards,
                    'active_count' => $active_gift_cards,
                    'expired_count' => $expired_gift_cards,
                    'total_issued' => $sold_gift_cards + $active_gift_cards + $expired_gift_cards
                ]
            ]
        ]);
    }
    ...
    /**
     * Get stock report chart data for graph visualization
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getStockReportChartData(Request $request)
    {
        if (! auth()->user()->can('product.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        // Get stock value by date - calculate stock value based on purchase and sale transactions
        // We'll calculate closing stock value for each date
        $query = Transaction::where('business_id', $business_id)
            ->whereIn('type', ['purchase', 'sell', 'purchase_return', 'sell_return'])
            ->where(function($q) {
                $q->where(function($q2) {
                    $q2->where('type', 'purchase')->where('status', 'received');
                })->orWhere(function($q2) {
                    $q2->where('type', 'sell')->where('status', 'final');
                })->orWhere(function($q2) {
                    $q2->where('type', 'purchase_return')->where('status', 'received');
                })->orWhere(function($q2) {
                    $q2->where('type', 'sell_return')->where('status', 'final');
                });
            })
            ->select(
                DB::raw('DATE(transaction_date) as date'),
                DB::raw('SUM(IF(type IN ("purchase", "sell_return"), final_total, 0)) - SUM(IF(type IN ("sell", "purchase_return"), final_total, 0)) as net_stock_value')
            );
        ...
        $results = $query->groupBy(DB::raw('DATE(transaction_date)'))
            ->orderBy('date', 'asc')
            ->get();

        // Calculate cumulative stock value
        $labels = [];
        $stockValues = [];
        $cumulativeValue = 0;

        foreach ($results as $result) {
            $cumulativeValue += (float) $result->net_stock_value;
            $labels[] = \Carbon\Carbon::parse($result->date)->format('M d');
            $stockValues[] = $cumulativeValue;
        }
    ...
    public function getStockDetails(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = Product::leftjoin('units as u', 'products.unit_id', '=', 'u.id')
                ...
                ->where('v.discontinued', false); // Exclude discontinued variations
    ...
    /**
     * Get tax report chart data for graph visualization
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getTaxReportChartData(Request $request)
    {
        if (! auth()->user()->can('tax_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $start_date = ! empty(request()->input('start_date')) ? request()->input('start_date') : $fy['start'];
        $end_date = ! empty(request()->input('end_date')) ? request()->input('end_date') : $fy['end'];
        $contact_id = ! empty(request()->input('contact_id')) ? request()->input('contact_id') : null;

        $permitted_locations = auth()->user()->permitted_locations();
        
        // Get input tax, output tax, and expense tax by date
        $inputTaxQuery = Transaction::where('business_id', $business_id)
            ->whereIn('type', ['purchase', 'purchase_return'])
            ->where('status', 'received');

        $outputTaxQuery = Transaction::where('business_id', $business_id)
            ->whereIn('type', ['sell', 'sell_return'])
            ->where('status', 'final');

        $expenseTaxQuery = Transaction::where('business_id', $business_id)
            ->where('type', 'expense')
            ->where('status', 'final');

        if ($permitted_locations != 'all') {
            $inputTaxQuery->whereIn('location_id', $permitted_locations);
            $outputTaxQuery->whereIn('location_id', $permitted_locations);
            $expenseTaxQuery->whereIn('location_id', $permitted_locations);
        }

        if (! empty($location_id)) {
            $inputTaxQuery->where('location_id', $location_id);
            $outputTaxQuery->where('location_id', $location_id);
            $expenseTaxQuery->where('location_id', $location_id);
        }

        if (! empty($contact_id)) {
            $inputTaxQuery->where('contact_id', $contact_id);
            $outputTaxQuery->where('contact_id', $contact_id);
            $expenseTaxQuery->where('contact_id', $contact_id);
        }

        if (! empty($start_date) && ! empty($end_date)) {
            $inputTaxQuery->whereDate('transaction_date', '>=', $start_date)
                  ->whereDate('transaction_date', '<=', $end_date);
            $outputTaxQuery->whereDate('transaction_date', '>=', $start_date)
                  ->whereDate('transaction_date', '<=', $end_date);
            $expenseTaxQuery->whereDate('transaction_date', '>=', $start_date)
                  ->whereDate('transaction_date', '<=', $end_date);
        }

        // Get input tax totals by date
        $inputTaxData = $inputTaxQuery->select(
            DB::raw('DATE(transaction_date) as date'),
        ...
        // Get output tax totals by date
        $outputTaxData = $outputTaxQuery->select(
            DB::raw('DATE(transaction_date) as date'),
            DB::raw('SUM(tax_amount) as total_tax')
        )
        ->groupBy(DB::raw('DATE(transaction_date)'))
        ...
        // Get expense tax totals by date
        $expenseTaxData = $expenseTaxQuery->select(
            DB::raw('DATE(transaction_date) as date'),
            DB::raw('SUM(tax_amount) as total_tax')
        )
        ->groupBy(DB::raw('DATE(transaction_date)'))
        ...
        // Combine dates and create labels
        $allDates = collect($inputTaxData->pluck('date'))
            ->merge($outputTaxData->pluck('date'))
            ->merge($expenseTaxData->pluck('date'))
            ->unique()
            ->sort()
            ->values();
        ...
        $inputTaxMap = $inputTaxData->pluck('total_tax', 'date')->toArray();
        $outputTaxMap = $outputTaxData->pluck('total_tax', 'date')->toArray();
        $expenseTaxMap = $expenseTaxData->pluck('total_tax', 'date')->toArray();

        foreach ($allDates as $date) {
            $labels[] = \Carbon\Carbon::parse($date)->format('M d');
            $inputTaxValues[] = isset($inputTaxMap[$date]) ? (float) $inputTaxMap[$date] : 0;
            $outputTaxValues[] = isset($outputTaxMap[$date]) ? (float) $outputTaxMap[$date] : 0;
            $expenseTaxValues[] = isset($expenseTaxMap[$date]) ? (float) $expenseTaxMap[$date] : 0;
        }

        return response()->json([
            'success' => true,
            'labels' => $labels,
            'input_tax' => $inputTaxValues,
            'output_tax' => $outputTaxValues,
            'expense_tax' => $expenseTaxValues
        ]);
    }
    ...
    /**
     * Get stock adjustment report chart data for graph visualization
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getStockAdjustmentChartData(Request $request)
    {
        if (! auth()->user()->can('stock_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        // Get stock adjustment data grouped by date
        $query = Transaction::where('business_id', $business_id)
            ->where('type', 'stock_adjustment')
            ->select(
                DB::raw('DATE(transaction_date) as date'),
        ...
        foreach ($results as $result) {
            $labels[] = \Carbon\Carbon::parse($result->date)->format('M d');
            $normalValues[] = (float) $result->total_normal;
            $abnormalValues[] = (float) $result->total_abnormal;
            $totalValues[] = (float) $result->total_amount;
            $recoveredValues[] = (float) $result->total_recovered;
        }
    ...
    /**
     * Get register report chart data for graph visualization
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getRegisterReportChartData(Request $request)
    {
        if (! auth()->user()->can('register_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        // Get cash register transaction data grouped by date
        $query = \App\CashRegisterTransaction::join('cash_registers as cr', 'cash_register_transactions.cash_register_id', '=', 'cr.id')
            ->where('cr.business_id', $business_id)
            ->where('cash_register_transactions.transaction_type', 'sell')
            ->select(
                DB::raw('DATE(cash_register_transactions.created_at) as date'),
                DB::raw("SUM(IF(pay_method='cash', amount, 0)) as total_cash"),
                DB::raw("SUM(IF(pay_method='card', amount, 0)) as total_card"),
                DB::raw("SUM(IF(pay_method='cheque', amount, 0)) as total_cheque"),
                DB::raw("SUM(IF(pay_method='bank_transfer', amount, 0)) as total_bank_transfer"),
                DB::raw("SUM(IF(pay_method IN ('advance', 'custom_pay_1', 'custom_pay_2', 'custom_pay_3', 'custom_pay_4', 'custom_pay_5', 'custom_pay_6', 'custom_pay_7'), amount, 0)) as total_other"),
                DB::raw('SUM(amount) as total_payment')
            );

        if ($permitted_locations != 'all') {
            $query->whereIn('cr.location_id', $permitted_locations);
        }

        if (! empty($location_id)) {
            $query->where('cr.location_id', $location_id);
        }
        ...
        if (! empty($start_date) && ! empty($end_date)) {
            $query->whereDate('cash_register_transactions.created_at', '>=', $start_date)
                  ->whereDate('cash_register_transactions.created_at', '<=', $end_date);
        }

        $results = $query->groupBy(DB::raw('DATE(cash_register_transactions.created_at)'))
            ->orderBy('date', 'asc')
            ->get();
        ...
        foreach ($results as $result) {
            $labels[] = \Carbon\Carbon::parse($result->date)->format('M d');
            $cashValues[] = (float) $result->total_cash;
            $cardValues[] = (float) $result->total_card;
            $chequeValues[] = (float) $result->total_cheque;
            $bankTransferValues[] = (float) $result->total_bank_transfer;
            $totalValues[] = (float) $result->total_payment;
        }
        ...
        return response()->json([
            ...
            'card' => $cardValues,
            'cheque' => $chequeValues,
            'bank_transfer' => $bankTransferValues,
            'total' => $totalValues
        ]);
    }
    ...
    /**
     * Get sales representative report chart data for graph visualization
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getSalesRepresentativeChartData(Request $request)
    {
        if (! auth()->user()->can('sales_representative.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $start_date = ! empty(request()->input('start_date')) ? request()->input('start_date') : $fy['start'];
        $end_date = ! empty(request()->input('end_date')) ? request()->input('end_date') : $fy['end'];
        $created_by = ! empty(request()->input('created_by')) ? request()->input('created_by') : null;

        $permitted_locations = auth()->user()->permitted_locations();
        
        // Get sales data grouped by date
        $salesQuery = Transaction::where('business_id', $business_id)
            ->where('type', 'sell')
            ->where('status', 'final');
        ...
        if (! empty($created_by)) {
            $salesQuery->where('created_by', $created_by);
        }

        if (! empty($start_date) && ! empty($end_date)) {
            $salesQuery->whereDate('transaction_date', '>=', $start_date)
                  ->whereDate('transaction_date', '<=', $end_date);
        }
        ...
        $salesReturnQuery = Transaction::where('business_id', $business_id)
            ->where('type', 'sell_return')
            ->where('status', 'final');

        if ($permitted_locations != 'all') {
            $salesReturnQuery->whereIn('location_id', $permitted_locations);
        }

        if (! empty($location_id)) {
            $salesReturnQuery->where('location_id', $location_id);
        }

        if (! empty($created_by)) {
            $salesReturnQuery->where('created_by', $created_by);
        }

        if (! empty($start_date) && ! empty($end_date)) {
            $salesReturnQuery->whereDate('transaction_date', '>=', $start_date)
                  ->whereDate('transaction_date', '<=', $end_date);
        }

        $salesReturnData = $salesReturnQuery->select(
            DB::raw('DATE(transaction_date) as date'),
            DB::raw('SUM(final_total) as total_returns')
        )
        ->groupBy(DB::raw('DATE(transaction_date)'))
        ->orderBy('date', 'asc')
        ->get();
        ...
        // Get expense data
        $expenseQuery = Transaction::where('business_id', $business_id)
            ->where('type', 'expense')
            ->where('status', 'final');

        if ($permitted_locations != 'all') {
            $expenseQuery->whereIn('location_id', $permitted_locations);
        }

        if (! empty($location_id)) {
            $expenseQuery->where('location_id', $location_id);
        }

        if (! empty($created_by)) {
            $expenseQuery->where('created_by', $created_by);
        }

        if (! empty($start_date) && ! empty($end_date)) {
            $expenseQuery->whereDate('transaction_date', '>=', $start_date)
                  ->whereDate('transaction_date', '<=', $end_date);
        }
        ...
        // Combine dates and create labels
        $allDates = collect($salesData->pluck('date'))
            ->merge($salesReturnData->pluck('date'))
        ...
        $salesMap = $salesData->pluck('total_sales', 'date')->toArray();
        $salesReturnMap = $salesReturnData->pluck('total_returns', 'date')->toArray();
        $expenseMap = $expenseData->pluck('total_expenses', 'date')->toArray();

        foreach ($allDates as $date) {
            $labels[] = \Carbon\Carbon::parse($date)->format('M d');
            $sales = isset($salesMap[$date]) ? (float) $salesMap[$date] : 0;
            $returns = isset($salesReturnMap[$date]) ? (float) $salesReturnMap[$date] : 0;
            $expenses = isset($expenseMap[$date]) ? (float) $expenseMap[$date] : 0;
            
            $salesValues[] = $sales;
            $salesReturnValues[] = $returns;
            $expenseValues[] = $expenses;
            $netSalesValues[] = $sales - $returns;
        }
        ...
        return response()->json([
            ...
            'labels' => $labels,
            'sales' => $salesValues,
            'sales_return' => $salesReturnValues,
            'expenses' => $expenseValues,
            'net_sales' => $netSalesValues
        ]);
    }
    ...
    /**
     * Get customer groups report chart data for graph visualization
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getCustomerGroupChartData(Request $request)
    {
        if (! auth()->user()->can('contacts_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $start_date = ! empty(request()->input('start_date')) ? request()->input('start_date') : $fy['start'];
        $end_date = ! empty(request()->input('end_date')) ? request()->input('end_date') : $fy['end'];
        $customer_group_id = ! empty(request()->input('customer_group_id')) ? request()->input('customer_group_id') : null;

        $permitted_locations = auth()->user()->permitted_locations();
        
        // Get sales totals by customer group
        $query = Transaction::leftjoin('customer_groups AS CG', 'transactions.customer_group_id', '=', 'CG.id')
            ->where('transactions.business_id', $business_id)
            ->where('transactions.type', 'sell')
            ->where('transactions.status', 'final')
        ...
        if (! empty($customer_group_id)) {
            $query->where('transactions.customer_group_id', $customer_group_id);
        }
        ...
        $results = $query->orderBy('total_sell', 'desc')
            ->get();
        ...
        foreach ($results as $result) {
            $labels[] = $result->group_name ?: __('lang_v1.uncategorized');
            $values[] = (float) $result->total_sell;
        }
    ...
    /**
     * Get product purchase report chart data for graph visualization
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getProductPurchaseReportChartData(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $brand_id = ! empty(request()->input('brand_id')) ? request()->input('brand_id') : null;

        $permitted_locations = auth()->user()->permitted_locations();
        
        // Get product purchase data grouped by date
        $query = PurchaseLine::join('transactions as t', 'purchase_lines.transaction_id', '=', 't.id')
            ...
            ->select(
                ...
                DB::raw('SUM((purchase_lines.quantity - purchase_lines.quantity_returned - purchase_lines.quantity_adjusted) * purchase_lines.purchase_price_inc_tax) as total_purchase_value'),
                DB::raw('AVG(purchase_lines.purchase_price_inc_tax) as avg_unit_price')
            );
        ...
        $results = $query->groupBy(DB::raw('DATE(t.transaction_date)'))
            ->orderBy('date', 'asc')
            ->get();
        ...
        foreach ($results as $result) {
            $labels[] = \Carbon\Carbon::parse($result->date)->format('M d');
            $quantityValues[] = (float) $result->total_quantity;
            $purchaseValueValues[] = (float) $result->total_purchase_value;
            $avgPriceValues[] = (float) $result->avg_unit_price;
        }
        ...
        return response()->json([
            ...
            'purchase_value' => $purchaseValueValues,
            'avg_unit_price' => $avgPriceValues
        ]);
    }

    /**
     * Get product purchase report pie chart data (grouped by product)
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getProductPurchaseReportPieChartData(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $query = PurchaseLine::join('transactions as t', 'purchase_lines.transaction_id', '=', 't.id')
            ...
            ->groupBy('p.id', 'p.name')
            ->orderBy('total_purchase_value', 'desc')
        ...
        foreach ($results as $result) {
            ...
            $values[] = (float) $result->total_purchase_value;
        }
    ...
    /**
     * Get product sell report chart data for graph visualization
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getProductSellReportChartData(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $start_date = ! empty(request()->input('start_date')) ? request()->input('start_date') : $fy['start'];
        $end_date = ! empty(request()->input('end_date')) ? request()->input('end_date') : $fy['end'];
        $customer_id = ! empty(request()->input('customer_id')) ? request()->input('customer_id') : null;
        $customer_group_id = ! empty(request()->input('customer_group_id')) ? request()->input('customer_group_id') : null;
        ...
        // Get product sell data grouped by date
        $query = TransactionSellLine::join('transactions as t', 'transaction_sell_lines.transaction_id', '=', 't.id')
            ...
            ->select(
                ...
                DB::raw('SUM((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax) as total_sale_value'),
                DB::raw('AVG(transaction_sell_lines.unit_price_inc_tax) as avg_unit_price')
            );
        ...
        foreach ($results as $result) {
            $labels[] = \Carbon\Carbon::parse($result->date)->format('M d');
            $quantityValues[] = (float) $result->total_quantity;
            $saleValueValues[] = (float) $result->total_sale_value;
            $avgPriceValues[] = (float) $result->avg_unit_price;
        }
        ...
        return response()->json([
            ...
            'labels' => $labels,
            'quantity' => $quantityValues,
            'sale_value' => $saleValueValues,
            'avg_unit_price' => $avgPriceValues
        ]);
    }

    /**
     * Get product sell report grouped bar chart data (grouped by product and date)
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getProductSellReportGroupedChartData(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        // Get product sell data grouped by date and product (top products per date)
        // First, get all dates in the range
        $dateQuery = \DB::table('transactions')
            ->where('business_id', $business_id)
            ->where('type', 'sell')
            ->where('status', 'final')
            ->select(DB::raw('DISTINCT DATE(transaction_date) as date'));

        if ($permitted_locations != 'all') {
            $dateQuery->whereIn('location_id', $permitted_locations);
        }

        if (! empty($location_id)) {
            $dateQuery->where('location_id', $location_id);
        }

        if (! empty($start_date) && ! empty($end_date)) {
            $dateQuery->whereDate('transaction_date', '>=', $start_date)
                      ->whereDate('transaction_date', '<=', $end_date);
        }

        $dates = $dateQuery->orderBy('date', 'asc')->pluck('date');

        // Get top 5 products by total sales value
        $topProductsQuery = TransactionSellLine::join('transactions as t', 'transaction_sell_lines.transaction_id', '=', 't.id')
            ->join('variations as v', 'transaction_sell_lines.variation_id', '=', 'v.id')
            ->join('product_variations as pv', 'v.product_variation_id', '=', 'pv.id')
            ->join('products as p', 'pv.product_id', '=', 'p.id')
            ->leftJoin('contacts as c', 't.contact_id', '=', 'c.id')
        ...
        if ($permitted_locations != 'all') {
            $topProductsQuery->whereIn('t.location_id', $permitted_locations);
        }

        if (! empty($location_id)) {
            $topProductsQuery->where('t.location_id', $location_id);
        }

        if (! empty($customer_id)) {
            $topProductsQuery->where('t.contact_id', $customer_id);
        }

        if (! empty($customer_group_id)) {
            $topProductsQuery->where('CG.id', $customer_group_id);
        }

        if (! empty($category_id)) {
            $topProductsQuery->where('p.category_id', $category_id);
        }

        if (! empty($brand_id)) {
            $topProductsQuery->where('p.brand_id', $brand_id);
        }

        if (! empty($start_date) && ! empty($end_date)) {
            $topProductsQuery->whereDate('t.transaction_date', '>=', $start_date)
                             ->whereDate('t.transaction_date', '<=', $end_date);
        }

        $topProducts = $topProductsQuery->get();
        $productIds = $topProducts->pluck('product_id')->toArray();
        $productNames = $topProducts->pluck('product_name', 'product_id')->toArray();

        // Get data for each product by date
        $datasets = [];
        $colors = [
            'rgb(255, 99, 132)',    // Red
            'rgb(54, 162, 235)',     // Blue
        ...
        foreach ($topProducts as $index => $product) {
            ...
            foreach ($dates as $date) {
                ...
                $result = $query->first();
                $productData[] = (float) ($result->total_sale_value ?? 0);
            }
    ...
    public function purchasePaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->addColumn('action', '<button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline text-white tw-dw-btn-primary view_payment" data-href="{{ action([\App\Http\Controllers\TransactionPaymentController::class, \'viewPayment\'], [$DT_RowId]) }}">@lang("messages.view")
    ...
    /**
     * Get purchase payment report chart data for graph visualization
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getPurchasePaymentReportChartData(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        // Build contact filter for supplier
        $contact_filter1 = ! empty($supplier_id) ? "AND t.contact_id=$supplier_id" : '';
        $contact_filter2 = ! empty($supplier_id) ? "AND transactions.contact_id=$supplier_id" : '';
        $parent_payment_query_part = empty($location_id) ? 'AND transaction_payments.parent_id IS NULL' : '';
        
        // Get purchase payment data grouped by date
        $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
                $join->on('transaction_payments.transaction_id', '=', 't.id')
                    ->where('t.business_id', $business_id)
                    ->whereIn('t.type', ['purchase', 'opening_balance']);
            ...
            ->select(
                DB::raw('DATE(transaction_payments.paid_on) as date'),
                ...
                DB::raw("SUM(IF(method NOT IN ('cash', 'card', 'cheque', 'bank_transfer'), amount, 0)) as total_other"),
                DB::raw('SUM(amount) as total_payment')
            );
        ...
        $results = $query->groupBy(DB::raw('DATE(transaction_payments.paid_on)'))
            ->orderBy('date', 'asc')
            ->get();
    ...
    /**
     * Get sell payment report chart data for graph visualization
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getSellPaymentReportChartData(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $customer_id = ! empty(request()->input('customer_id')) ? request()->input('customer_id') : null;
        $customer_group_id = ! empty(request()->input('customer_group_id')) ? request()->input('customer_group_id') : null;
        $payment_method = ! empty(request()->input('payment_types')) ? request()->input('payment_types') : null;

        $permitted_locations = auth()->user()->permitted_locations();
        
        // Build contact filter for customer
        $contact_filter1 = ! empty($customer_id) ? "AND t.contact_id=$customer_id" : '';
        $contact_filter2 = ! empty($customer_id) ? "AND transactions.contact_id=$customer_id" : '';
        $parent_payment_query_part = empty($location_id) ? 'AND transaction_payments.parent_id IS NULL' : '';
        
        // Get sell payment data grouped by date
        $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
                $join->on('transaction_payments.transaction_id', '=', 't.id')
                    ->where('t.business_id', $business_id)
                    ->whereIn('t.type', ['sell', 'opening_balance']);
            ...
            ->select(
                DB::raw('DATE(transaction_payments.paid_on) as date'),
                DB::raw("SUM(IF(method='cash', IF(is_return=1, -1*amount, amount), 0)) as total_cash"),
                DB::raw("SUM(IF(method='card', IF(is_return=1, -1*amount, amount), 0)) as total_card"),
                DB::raw("SUM(IF(method='cheque', IF(is_return=1, -1*amount, amount), 0)) as total_cheque"),
                DB::raw("SUM(IF(method='bank_transfer', IF(is_return=1, -1*amount, amount), 0)) as total_bank_transfer"),
                DB::raw("SUM(IF(method NOT IN ('cash', 'card', 'cheque', 'bank_transfer'), IF(is_return=1, -1*amount, amount), 0)) as total_other"),
                DB::raw('SUM(IF(is_return=1, -1*amount, amount)) as total_payment')
            );
        ...
        if (! empty($payment_method)) {
            $query->where('transaction_payments.method', $payment_method);
        }
    ...
    /**
     * Get items report chart data for graph visualization
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getItemsReportChartData(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $location_id = ! empty(request()->input('location_id')) ? request()->input('location_id') : null;
        $purchase_start = ! empty(request()->input('purchase_start')) ? request()->input('purchase_start') : $fy['start'];
        $purchase_end = ! empty(request()->input('purchase_end')) ? request()->input('purchase_end') : $fy['end'];
        $sale_start = ! empty(request()->input('sale_start')) ? request()->input('sale_start') : $fy['start'];
        $sale_end = ! empty(request()->input('sale_end')) ? request()->input('sale_end') : $fy['end'];
        $supplier_id = ! empty(request()->input('supplier_id')) ? request()->input('supplier_id') : null;
        $customer_id = ! empty(request()->input('customer_id')) ? request()->input('customer_id') : null;

        $permitted_locations = auth()->user()->permitted_locations();
        
        // Get items report data grouped by sale date
        $query = TransactionSellLinesPurchaseLines::leftJoin('transaction_sell_lines as SL', 'SL.id', '=', 'transaction_sell_lines_purchase_lines.sell_line_id')
            ...
            ->where('purchase.business_id', $business_id)
            ->select(
                DB::raw('DATE(COALESCE(sale.transaction_date, stock_adjustment.transaction_date)) as date'),
                DB::raw('SUM(PL.purchase_price_inc_tax * transaction_sell_lines_purchase_lines.quantity) as total_purchase_value'),
                DB::raw('SUM(COALESCE(SL.unit_price_inc_tax, SAL.unit_price) * transaction_sell_lines_purchase_lines.quantity) as total_sale_value'),
                DB::raw('SUM((COALESCE(SL.unit_price_inc_tax, SAL.unit_price) - PL.purchase_price_inc_tax) * transaction_sell_lines_purchase_lines.quantity) as total_profit')
            );
        ...
        $results = $query->whereNotNull(DB::raw('COALESCE(sale.transaction_date, stock_adjustment.transaction_date)'))
            ->groupBy(DB::raw('DATE(COALESCE(sale.transaction_date, stock_adjustment.transaction_date))'))
            ->orderBy('date', 'asc')
            ->get();
        ...
        foreach ($results as $result) {
            $labels[] = \Carbon\Carbon::parse($result->date)->format('M d');
            $purchaseValues[] = (float) $result->total_purchase_value;
            $saleValues[] = (float) $result->total_sale_value;
            $profitValues[] = (float) $result->total_profit;
        }

        return response()->json([
            'success' => true,
            'labels' => $labels,
            'purchase_value' => $purchaseValues,
    ...
    public function activityLog()
    {
        ...
        if (request()->ajax()) {
            ...
            $activities = Activity::with(['subject'])
                                ->leftjoin('users as u', 'u.id', '=', $activity_table.'.causer_id')
                                ->where($activity_table.'.business_id', $business_id)
                                ->select(
                                    $activity_table.'.*',
            ...
            if (! empty(request()->start_date) && ! empty(request()->end_date)) {
                $start = request()->start_date;
                $end = request()->end_date;
                $activities->whereDate($activity_table.'.created_at', '>=', $start)
                            ->whereDate($activity_table.'.created_at', '<=', $end);
            }
            ...
            if (! empty(request()->activity_action)) {
                $activities->where('description', request()->activity_action);
            }

            if (! empty(request()->activity_table)) {
                $activities->whereRaw(
                    "JSON_UNQUOTE(JSON_EXTRACT({$activity_table}.properties, '$.table')) = ?",
                    [request()->activity_table]
                );
            }
            ...
            return Datatables::of($activities)
                            ...
                            ->addColumn('subject_type', function ($row) use ($transaction_types) {
                                ...
                                if (empty($subject_type) && ! empty($row->subject_type)) {
                                    $subject_type = class_basename($row->subject_type);
                                }

                                if (empty($subject_type) && ! empty($row->getExtraProperty('module'))) {
                                    $subject_type = $row->getExtraProperty('module');
                                }

                                return $subject_type;
                            })
                            ->addColumn('note', function ($row) use ($statuses, $shipping_statuses, $transaction_types) {
                                $html = '';
                                $module = $row->getExtraProperty('module');
                                $table = $row->getExtraProperty('table');
                                $subject_id = $row->subject_id ?: $row->getExtraProperty('subject_id');
                                $action_key = 'lang_v1.'.$row->description;
                                $action_label = __($action_key);
                                if ($action_label === $action_key) {
                                    $action_label = ucfirst(str_replace('_', ' ', $row->description));
                                }
                                $subject_label = '';
                                if ($row->subject_type == \App\Contact::class) {
                                    $subject_label = __('contact.contact');
                                } elseif ($row->subject_type == \App\User::class) {
                                    $subject_label = __('report.user');
                                } elseif ($row->subject_type == \App\Transaction::class && ! empty($row->subject->type)) {
                                    $subject_label = $transaction_types[$row->subject->type] ?? '';
                                } elseif ($row->subject_type == \App\TransactionPayment::class) {
                                    $subject_label = __('lang_v1.payment');
                                } elseif (! empty($row->subject_type)) {
                                    $subject_label = class_basename($row->subject_type);
                                } elseif (! empty($module)) {
                                    $subject_label = $module;
                                }
                                $identifier = '';
                                if (! empty($row->getExtraProperty('name'))) {
                                    $identifier = __('user.name').': '.$row->getExtraProperty('name');
                                } elseif (! empty($row->subject->name)) {
                                    $identifier = __('user.name').': '.$row->subject->name;
                                } elseif (! empty($row->getExtraProperty('invoice_no'))) {
                                    $identifier = __('sale.invoice_no').': '.$row->getExtraProperty('invoice_no');
                                } elseif (! empty($row->subject->invoice_no)) {
                                    $identifier = __('sale.invoice_no').': '.$row->subject->invoice_no;
                                } elseif (! empty($row->getExtraProperty('ref_no'))) {
                                    $identifier = __('purchase.ref_no').': '.$row->getExtraProperty('ref_no');
                                } elseif (! empty($row->subject->ref_no)) {
                                    $identifier = __('purchase.ref_no').': '.$row->subject->ref_no;
                                } elseif (! empty($subject_id)) {
                                    $identifier = 'ID: '.$subject_id;
                                }
                                if (! empty($action_label)) {
                                    $sentence = trim($action_label.' '.$subject_label);
                                    if (! empty($identifier)) {
                                        $sentence .= ' ('.$identifier.')';
                                    }
                                    $html .= $sentence.'<br>';
                                ...
                                $summary_parts = [];
                                if (! empty($module)) {
                                    $summary_parts[] = 'Module: '.$module;
                                }
                                if (! empty($table)) {
                                    $summary_parts[] = 'Table: '.$table;
                                }
                                if (! empty($subject_id)) {
                                    $summary_parts[] = 'ID: '.$subject_id;
                                }
                                if (! empty($summary_parts)) {
                                    $html .= implode(' | ', $summary_parts).'<br>';
                                }
                                ...
                                if ($row->subject_type == \App\Transaction::class && ! empty($row->subject) && in_array($row->subject->type, ['sell', 'purchase'])) {
                                    ...
                                    if (! empty($update_note) && ! is_array($update_note)) {
                                        $html .= $update_note;
                                    } else if (! empty($row->getExtraProperty('customized'))) {
                                        $customized = $row->getExtraProperty('customized');
                                        $html .= view('sale_pos.partials.customized_activity_row', ['customized' => $customized])->render();
                                    }
                                }
                                ...
                                $extra_parts = [];
                                if ($row->description === 'accessed') {
                                    $route_name = $row->getExtraProperty('route_name');
                                    $path = $row->getExtraProperty('path');
                                    if (! empty($route_name)) {
                                        $extra_parts[] = 'Route: '.$route_name;
                                    } elseif (! empty($path)) {
                                        $extra_parts[] = 'Path: '.$path;
                                    }
                                }
                                if (! empty($row->getExtraProperty('invoice_no'))) {
                                    $extra_parts[] = __('sale.invoice_no').': '.$row->getExtraProperty('invoice_no');
                                }
                                if (! empty($row->getExtraProperty('ref_no'))) {
                                    $extra_parts[] = __('purchase.ref_no').': '.$row->getExtraProperty('ref_no');
                                }
                                $new = (array) $row->getExtraProperty('attributes');
                                if (! empty($new)) {
                                    $fields = array_keys($new);
                                    $shown = array_slice($fields, 0, 4);
                                    $remaining = count($fields) - count($shown);
                                    $changes_summary = 'Changes: '.implode(', ', $shown);
                                    if ($remaining > 0) {
                                        $changes_summary .= ' +'.$remaining.' more';
                                    }
                                    $extra_parts[] = $changes_summary;
                                }
                                if (! empty($extra_parts)) {
                                    $html .= implode(' | ', $extra_parts);
                                }
                            ...
                            ->editColumn('description', function ($row) use ($transaction_types) {
                                if (empty($row->description)) {
                                    return __('lang_v1.activity_log');
                                }
                                ...
                                $verb_map = [
                                    'created' => __('lang_v1.created'),
                                    'updated' => __('lang_v1.updated'),
                                    'deleted' => __('lang_v1.deleted'),
                                    'accessed' => __('lang_v1.accessed'),
                                    'login' => __('lang_v1.login'),
                                    'logout' => __('lang_v1.logout'),
                                ];
                                $verb = $verb_map[$row->description] ?? $action_label;

                                if ($row->description === 'login' || $row->description === 'logout') {
                                    return $verb;
                                }
                                ...
                                if ($row->description === 'accessed') {
                                    ...
                                    $route_name = $row->getExtraProperty('route_name');
                                    $path = $row->getExtraProperty('path');
                                    $target = $module ?: ($route_name ?: ($path ?: __('lang_v1.activity_log')));
                                ...
                                if (empty($subject_label) && ! empty($row->getExtraProperty('module'))) {
                                    $subject_label = $row->getExtraProperty('module');
                                }

                                if (empty($subject_label)) {
                                    $subject_label = __('lang_v1.activity_log');
                                }
                                ...
                                if (! empty($row->getExtraProperty('name'))) {
                                    ...
                                    $identifier = 'ID: '.$row->subject_id;
                                }

                                $sentence = trim($verb.' '.$subject_label);
                                if (! empty($identifier)) {
                                    $sentence .= ' ('.$identifier.')';
                                }
        ...
        return view('report.activity_log')->with(compact('users', 'transaction_types', 'actions'));
    }
    ...
    /**
     * Check if user has access to specific location type
     * 
     * @param string $locationType 'b2b' or 'b2c'
     * @return bool
     */
    private function hasLocationTypeAccess($locationType = 'b2b')
    {
        $user = auth()->user();
    ...
    {
        return $this->hasLocationTypeAccess('b2b');
    }
    ...
    {
        return $this->hasLocationTypeAccess('b2c');
    }
}
