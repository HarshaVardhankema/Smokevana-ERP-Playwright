...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        View::composer(
            ['layouts.*'],
            function ($view) {
                if (isAppInstalled() && !app()->runningInConsole()) {
                    try {
        ...
        Event::listen('eloquent.updating: *', function ($eventName, array $data) use ($activityLogService) {
            $model = $data[0] ?? null;
            if ($model) {
                $activityLogService->storeOriginal($model);
            }
        });

        Event::listen('eloquent.created: *', function ($eventName, array $data) use ($activityLogService) {
            $model = $data[0] ?? null;
            if ($model) {
                $activityLogService->logModelEvent($model, 'created');
            }
        });

        Event::listen('eloquent.updated: *', function ($eventName, array $data) use ($activityLogService) {
            $model = $data[0] ?? null;
            if ($model) {
                $activityLogService->logModelEvent($model, 'updated');
            }
        });

        Event::listen('eloquent.deleted: *', function ($eventName, array $data) use ($activityLogService) {
            $model = $data[0] ?? null;
            if ($model) {
                $activityLogService->logModelEvent($model, 'deleted');
            }
        });
