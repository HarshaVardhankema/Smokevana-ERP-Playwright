...
class ContactUtil extends Util
{
    ...
    public function updateContact($input, $id, $business_id ,$is_change_approve=false)
    {
        $count = 0;
        //Check Contact id
        if (! empty($input['contact_id'])) {
        ...
        if (! empty($input['customer_u_name'])) {
            $count = Contact::where('business_id', $business_id)
                    ->where('customer_u_name', $input['customer_u_name'])
                    ->where('id', '!=', $id)
                    ->count();
        }
        ...
        if ($count == 0) {
            ...
            // Store password before it's applied to contact (for WpVendor sync)
            $passwordToSync = $input['password'] ?? null;
            ...
            // Sync password to WpVendor if this is a dropship vendor
            if (!empty($passwordToSync) && $contact->type == 'supplier' && !empty($contact->email)) {
                $wpVendor = WpVendor::where('email', $contact->email)
                    ->where('business_id', $business_id)
                    ->whereIn('vendor_type', [WpVendor::TYPE_ERP_DROPSHIP, WpVendor::TYPE_ERP])
                    ->first();
                    
                if ($wpVendor) {
                    $wpVendor->password = $passwordToSync; // Already hashed from ContactController
                    $wpVendor->save();
                    \Log::info('WpVendor password synced', ['vendor_id' => $wpVendor->id, 'email' => $contact->email]);
                }
            }
    ...
    public function getContactQuery($business_id, $type, $contact_ids = [], $location_id = null)
    {
        ...
        if (empty($location_id) && auth()->check()) {
            $location_id = Contact::getAutoDetectedLocation($business_id);
        }
