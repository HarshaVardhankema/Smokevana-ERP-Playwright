...
class ProductUtil extends Util
{
     ...
     * @return bool
     */
    public function createSingleProductVariation($product, $sku, $purchase_price, $dpp_inc_tax, $profit_percent, $selling_price, $selling_price_inc_tax, $combo_variations = [], $barcode_no = null)
    {
        ...
        if ($barcode_no) {
            $variation_data['var_barcode_no'] = $barcode_no;
        }
        ...
        foreach ($input_variations as $key => $value) {
            ...
            if (! empty($value['variations'])) {
                ...
                foreach ($value['variations'] as $k => $v) {
                    ...
                    //custom start
                    // if($product->barcode_no == '' || $product->barcode_no == null){
                    //     $$var_barcode_no= $v['var_barcode_no']?? null;
                    // } else {
                    // $var_barcode_no = empty($v['var_barcode_no']) ? $this->generateVarBarCode($product->barcode_no, $c, $product->barcode_type, $v['value'], $sku_type) : $v['var_barcode_no'];
                    $var_barcode_no = !empty($v['var_barcode_no']) ? $v['var_barcode_no'] : null;
                    // }
                    $var_maxSaleLimit = empty($v['var_maxSaleLimit']) ? null : $v['var_maxSaleLimit'];
                    ...
                    $variation_data[] = [
                        ...
                        'var_maxSaleLimit' => $var_maxSaleLimit,
                ...
                foreach ($variations as $variation) {
                    ...
                    // If var_maxSaleLimit is set, trigger cart adjustment for new variation
                    if (!empty($variation->var_maxSaleLimit)) {
                        $productController = new \App\Http\Controllers\ProductController(new ProductUtil(), new ModuleUtil());
                        $productController->updateCartItemsForNewLimit(null, $variation->product_id, $variation->var_maxSaleLimit);
                    }
    ...
    public function updateVariableProductVariations($product_id, $input_variations_edit, $sku_type)
    {
        ...
        foreach ($input_variations_edit as $key => $value) {
            ...
            if (! empty($value['variations_edit'])) {
                foreach ($value['variations_edit'] as $k => $v) {
                    ...
                    if (! empty($v['var_barcode_no'])) {
                        $data['var_barcode_no'] = $v['var_barcode_no'] ?? null;
                    } else {
                        $data['var_barcode_no'] = null;
                    }
                    $data['var_maxSaleLimit'] = empty($v['var_maxSaleLimit']) ? null : $v['var_maxSaleLimit'];

                    $variation = Variation::where('id', $k)
                        ->where('product_variation_id', $key)
                        ->first();

                    // Check if var_maxSaleLimit changed and trigger cart adjustment
                    $oldMaxSaleLimit = $variation->var_maxSaleLimit;
                    $newMaxSaleLimit = $data['var_maxSaleLimit'] ?? null;
                    ...
                    if ($oldMaxSaleLimit != $newMaxSaleLimit && !empty($newMaxSaleLimit)) {
                        $productController = new \App\Http\Controllers\ProductController(new ProductUtil(), new ModuleUtil());
                        $productController->updateCartItemsForNewLimit(null, $variation->product_id, $newMaxSaleLimit);
                    }

                    // Update group prices (Vip, Diamond, Silver, Gold, Platinum, etc.)
                    if (!empty($v['group_prices']) && is_array($v['group_prices'])) {
                        foreach ($v['group_prices'] as $pg_id => $pv) {
                            $price_value = is_array($pv) ? (isset($pv['price']) ? $pv['price'] : (isset($pv['price_inc_tax']) ? $pv['price_inc_tax'] : null)) : $pv;
                            if ($price_value === '' || $price_value === null) {
                                \App\VariationGroupPrice::where('variation_id', $variation->id)
                                    ->where('price_group_id', $pg_id)
                                    ->delete();
                                continue;
                            }
                            $price = $this->num_uf($price_value);
                            $price_type = (is_array($pv) && isset($pv['price_type'])) ? $pv['price_type'] : 'fixed';
                            if ($price > 0) {
                                \App\VariationGroupPrice::updateOrCreate(
                                    ['price_group_id' => $pg_id, 'variation_id' => $variation->id],
                                    ['price_inc_tax' => $price, 'price_type' => $price_type]
                                );
                            } else {
                                \App\VariationGroupPrice::where('variation_id', $variation->id)
                                    ->where('price_group_id', $pg_id)
                    ...
                    // Only upload media if files are present - don't remove existing media if no new files
                    if (request()->hasFile('edit_variation_images_' . $key . '_' . $k)) {
            ...
            if (! empty($value['variations'])) {
                ...
                foreach ($new_variations as $new_variation) {
                    ...
                    // If var_maxSaleLimit is set, trigger cart adjustment for new variation
                    if (!empty($new_variation->var_maxSaleLimit)) {
                        $productController = new \App\Http\Controllers\ProductController();
                        $productController->updateCartItemsForNewLimit(null, $new_variation->product_id, $new_variation->var_maxSaleLimit);
                    }
        ...
        // Get explicitly discontinued variations from request
        $discontinued_variation_ids = request()->input('discontinued_variations', []);
        
        // Mark explicitly discontinued variations
        if (!empty($discontinued_variation_ids)) {
            Variation::whereIn('id', $discontinued_variation_ids)
                ->where('product_id', $product->id)
                ->update(['discontinued' => true]);
        }
        
        // Get removed variations (not in form submission)
        ...
        foreach ($removed_variations as $removed_variation_id) {
            // Check if purchase or sell exist for the variation
            ...
            $count_sales_order = TransactionSellLine::join(
                'transactions as T',
                'transaction_sell_lines.transaction_id',
                '=',
                'T.id'
            ...
            // If purchase, sell, or sales_order exist, mark as discontinued instead of deleting
            if ($count_purchase > 0 || $count_sell > 0 || $count_sales_order > 0) {
    ...
    public function updateProductQuantity($location_id, $product_id, $variation_id, $new_quantity, $old_quantity = 0, $number_format = null, $uf_data = true)
    {
        ...
        if ($product->enable_stock == 1 && $qty_difference != 0) {
            ...
            if (empty($variation_location_d)) {
                ...
                $variation_location_d->in_stock_qty = 0;
            }
            
            // Update hand stock
            $variation_location_d->qty_available += $qty_difference;
            $old_in_stock_qty = $variation_location_d->in_stock_qty;
            // Calculate virtual stock correctly based on hand stock and remaining sales orders
            if ($variation_location_d->qty_available < 0) {
                // When hand stock is negative, virtual stock should be 0
                $variation_location_d->in_stock_qty = 0;
            } else {
                // When hand stock is positive, calculate virtual stock as available minus remaining orders
                $remaining_ordered = DB::table('transaction_sell_lines as tsl')
                    ...
                    ->where(function ($q) {
                        $q->whereNotIn('t.picking_status', ['INVOICED'])
                            ->orWhereNull('t.picking_status');
                    })
                    ->sum('tsl.ordered_quantity');
                
                $variation_location_d->in_stock_qty = max(0, $variation_location_d->qty_available - $remaining_ordered);
            }
            
            $variation_location_d->save();

            // Sync combo product stock
            $this->updateComboProductStock($variation_id, $location_id);

            if ($qty_difference > 0 && $variation_location_d->in_stock_qty > 0 && $old_in_stock_qty <= 0) {
                $this->triggerStockAlert($product_id, $variation_id, $product->business_id);
            }
        }
        return true;
    }
     /**
     * Trigger stock alert for a product
     */
    private function triggerStockAlert($product_id, $variation_id, $business_id)
    {
        try {
            // Get the stock alert controller and send alerts
            $controller = new \App\Http\Controllers\StockAlertController();
            $controller->sendStockAlerts($product_id, $business_id);
        } catch (\Exception $e) {
            \Log::error('Failed to trigger stock alert: ' . $e->getMessage());
        }
    }
     ...
     * @return bool
     */
    public function decreaseProductQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0, $reduceBoth = true)
    {
        $qty_difference = $new_quantity - $old_quantity;
        ...
        if ($product->enable_stock == 1) {
            ...
            try {
                if ($reduceBoth == true) {
                    $temp = (int) $qty_difference;
                    $details->decrement('in_stock_qty', $temp); // custom start end col qty reduce 
                }
            } catch (\Throwable $e) {
                $details->update(['in_stock_qty' => 0]);
                Log::emergency('File:' . $e->getFile() . 'Line:' . $e->getLine() . ' Fail to decrease qty Message:' . $e->getMessage());
            }
        }
    ...
    public function decreaseProductInStock($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0, $reduceBoth = true)
    {
        $qty_difference = $new_quantity - $old_quantity;
        ...
        if ($product->enable_stock == 1) {
            ...
            // Always decrement qty_available so product list (current stock) updates
            try {
                $details->decrement('qty_available', $qty_difference);
            } catch (\Throwable $e) {
                \Log::emergency('File:' . $e->getFile() . 'Line:' . $e->getLine() . ' Fail to decrease qty Message:' . $e->getMessage());
    ...
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true)
    {
        ...
        $query = Variation::join('products AS p', 'variations.product_id', '=', 'p.id')
            ...
            ->leftjoin('variation_group_prices AS vgp', 'variations.id', '=', 'vgp.variation_id') //custom
            ...
            ->where('variations.discontinued', false); // Exclude discontinued variations
        ...
        $product = $query->select(
            ...
            DB::raw("CASE 
                        WHEN p.type = 'variable' THEN (
                            SELECT GROUP_CONCAT(
                                CONCAT(spg.name, ':', vgp.price_inc_tax) 
                                SEPARATOR ', '
                            )
                            FROM variation_group_prices as vgp
                            JOIN selling_price_groups as spg 
                                ON vgp.price_group_id = spg.id
                            WHERE vgp.variation_id = variations.id
                        )
                        ELSE (
                            SELECT GROUP_CONCAT(
                                CONCAT(spg.name, ':', vgp.price_inc_tax) 
    ...
    /**
     * Helper method to handle overselling logic for in_stock_qty calculation
     */
    private function handleOversellingInStockQty($details, $transaction, $product_variation_id, $product_id, $qtyChange)
    {
        if ($details->qty_available < 0 && $details->in_stock_qty == 0) {
            $remaining = TransactionSellLine::with(['transaction'])
                ...
                ->where('variation_id', $product_variation_id)
                ->where('product_id', $product_id)
                ->sum('ordered_quantity');

            $add_to_stock = $qtyChange - $remaining;
            if ($remaining == 0) {
                try {
                    $details->in_stock_qty = $details->qty_available;
                } catch (\Throwable $th) {
                    $details->in_stock_qty = 0;
                }
            } else if ($add_to_stock <= 0) {
                $details->in_stock_qty = 0;
            } else {
                $details->in_stock_qty = $add_to_stock;
            }
        } else {
            $remaining = TransactionSellLine::with(['transaction'])
                ->whereHas('transaction', function ($query) use ($transaction) {
                ...
                ->where('product_id', $product_id)
                ->sum('ordered_quantity');
            $details->in_stock_qty += $details->qty_available - $remaining;
        }
    }
    ...
    public function adjustProductStockForInvoice($status_before, $transaction, $input, $uf_data = true)
    {
        if ($status_before == 'final' && $transaction->status == 'draft') {
        ...
        } else if ($transaction->type == "sales_order" && $transaction->status == 'ordered') {
            // erp custom logic 
            foreach ($input['products'] as $product) {
                $details = VariationLocationDetails::where('variation_id', $product['variation_id'])
                    ...
                    ->first();
                $old = TransactionSellLine::where('product_id', $product['product_id'])
                    ->where('transaction_id', $transaction->id)
                    ->where('variation_id', $product['variation_id'])
                    ->first();
                if ($old) {
                    $qtyChange = ($old->quantity ?? 0) - (int)$product['quantity'];

                    if ($qtyChange != 0) {
                        if ($transaction->picking_status == 'PICKED') {
                            // For quantity changes in PICKED/PICKING status

                            if ($qtyChange < 0) {
                                // If quantity is increased (e.g., 20 to 30), qtyChange will be -10
                                // We need to reduce available quantities by 10
                                if ($transaction->picking_status == 'PICKED' || $transaction->picking_started_at != null) {
                                    $details->qty_available -= abs($qtyChange);
                                }
                                try {
                                    $details->in_stock_qty -= abs($qtyChange);
                                } catch (\Exception $e) {
                                    $details->in_stock_qty = 0;
                                }
                            } else {
                                // If quantity is decreased (e.g., 30 to 20), qtyChange will be 10
                                // We need to increase available quantities by 10
                                $stock_val =  $details->qty_available;
                                if ($transaction->picking_status == 'PICKED' || $transaction->picking_started_at != null) {
                                    $details->qty_available += $qtyChange;
                                }
                                ...
                                if ($details->qty_available < 0 && $details->in_stock_qty == 0) {
                                    ...
                                    $remaining = TransactionSellLine::with(['transaction'])
                                        ...
                                        ->where('product_id', $product['product_id'])
                                        // ->sum('ordered_quantity');
                                        ->sum(DB::raw('ordered_quantity - IFNULL(picked_quantity,0)'));
                                    // dd($qtyChange - $remaining);
                                    // dd('Web:'.$details->in_stock_qty, 'change:'.$qtyChange , 'remaining:'.$remaining, 'hand:'.$details->qty_available);
                                    // if($transaction->picking_status == 'PICKED'){
                                    $details->in_stock_qty = $details->qty_available - $remaining;
                                    // dd('Web:'.$details->in_stock_qty, 'hand:'.$details->qty_available);
                                    // }else {
                                    //     $details->in_stock_qty += $qtyChange - $remaining;
                                    // }
                                    // $details->in_stock_qty += $qtyChange ;

                                }
                            }

                            $old->ordered_quantity += - ($qtyChange);
                            $old->quantity += - ($qtyChange);
                            if ($transaction->picking_started_at != null) {
                                $old->picked_quantity += - ($qtyChange);
                                $old->manual_picked_qty += - ($qtyChange);
                                if ($transaction->picking_status == 'PICKED') {
                                    $old->verified_qty += - ($qtyChange);
                                }
                            }
                            // dd('$old->picked_quantity:'.$old->picked_quantity, '$old->manual_picked_qty:'.$old->manual_picked_qty, '$old->verified_qty:'.$old->verified_qty, '$details->in_stock_qty:'.$details->in_stock_qty, '$details->qty_available:'.$details->qty_available);
                            $old->save();
                        } else {
                            if ($qtyChange < 0) {
                                try {
                                ...
                                if ($details->qty_available < 0 && $details->in_stock_qty == 0) {
                                    ...
                                    if ($details->qty_available > 0) {
                                        // 100  0 
                                        // qtyChange => 110 to 100 so -> 10 
                                        // 
                                        $remaining = TransactionSellLine::with(['transaction'])
                                            ->whereHas('transaction', function ($query) use ($transaction) {
                                                $query->where('type', 'sales_order')
                                            ...
                                            ->sum('ordered_quantity');
                                        // $details->in_stock_qty == 0
                                        if ($details->in_stock_qty == 0) {
                                            if ($details->qty_available < $product['quantity']) {
                                                $details->in_stock_qty = 0;
                                            } else {
                                                try {
                                                    $details->in_stock_qty = $details->qty_available - $product['quantity'] - $remaining;
                                                    // $details->in_stock_qty += $qtyChange - $remaining;
                                                } catch (\Exception $e) {
                                                    $details->in_stock_qty = 0;
                                                }
                                        ...
                                        // dd($details->in_stock_qty,,$qtyChange,$);
                                    } else {
                                    }
                                }
                                // $details->in_stock_qty += $qtyChange;
                            ...
                            // dd($details->qty_available,$details->in_stock_qty,$old->quantity);
                            $old->save();
                            $details->save();
                        }
                    }
                    ...
                    if ($transaction->picking_status == 'PICKED') {
                        // For new lines in PICKED/PICKING status, reduce available quantity immediately
                        $details->qty_available -= (int)$product['quantity'];
                        try {
                            $details->in_stock_qty -= (int)$product['quantity'];
                        } catch (\Exception $e) {
                            $details->in_stock_qty = 0;
                        }
                        $details->save();
                    } else {
                        // only in_stock qty will be updated
                        try {
                            $details->in_stock_qty -= (int)$product['quantity'];
                        } catch (\Exception $e) {
                            $details->in_stock_qty = 0;
            ...
            // ERP custom logic Handle removed lines
            $existing_lines = TransactionSellLine::where('transaction_id', $transaction->id)
                ->pluck('id')
                ->toArray();
            $current_lines = collect($input['products'])->pluck('transaction_sell_lines_id')
                ->filter()
                ->toArray();
            $removed_lines = array_diff($existing_lines, $current_lines);

            if (!empty($removed_lines)) {
                $removed_sell_lines = TransactionSellLine::whereIn('id', $removed_lines)->get();
                foreach ($removed_sell_lines as $removed_line) {
                    $details = VariationLocationDetails::where('variation_id', $removed_line->variation_id)
                        ->where('product_id', $removed_line->product_id)
                        ->where('location_id', $input['location_id'])
                        ->first();
                    if ($details && ($transaction->picking_status == 'PICKED')) {
                        // Restore quantities only for the removed line
                        // $details->qty_available += $removed_line->quantity;
                        try {

                            // Only restore if the line was actually picked
                            if ($removed_line->picked_quantity > 0) {
                                $details->in_stock_qty += $removed_line->quantity;
                                $details->qty_available += $removed_line->picked_quantity; // Keep qty_available in sync with in_stock_qty
                        ...
                        // Only restore in_stock_qty, do NOT touch qty_available
                        try {
                            if ($details->in_stock_qty == 0) {
                                // if($details->qty_available < $product['quantity']){
                                //     $details->in_stock_qty = 0;
                                // } else {
                                try {
                                    $a = $details->qty_available; //100
                                    $b = $removed_line->quantity; // 40
                                    // orignal stock 
                                    if (($a - $b) < 0) {
                                        if ($details->qty_available > 0) {
                                            $details->in_stock_qty = $details->qty_available;
                                        ...
                                        // dd($details->in_stock_qty, $stock_val, $remaining , $details->in_stock_qty+);
                                        $details->in_stock_qty = $details->qty_available - $remaining;
                                        // $details->in_stock_qty = 0;
                                    }
                                    // $details->in_stock_qty += $qtyChange - $remaining;
                        ...
                        // dd($details->in_stock_qty,$details->qty_available);  
                        $details->save();
                    }
                }
            }
    ...
    public function generateVarBarCode($sku, $c, $barcode_type, $value, $sku_type)
    {
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            // Normalize price keys so missing form fields (e.g. purchase_price) don't cause "Undefined array key"
            $data['pp_without_discount'] = $data['pp_without_discount'] ?? $data['purchase_price'] ?? 0;
            $data['purchase_price'] = $data['purchase_price'] ?? $data['pp_without_discount'] ?? 0;
            $data['purchase_price_inc_tax'] = $data['purchase_price_inc_tax'] ?? $data['purchase_price'] ?? $data['pp_without_discount'] ?? 0;
            $data['discount_percent'] = $data['discount_percent'] ?? 0;
            $data['item_tax'] = $data['item_tax'] ?? 0;
            $data['purchase_line_tax_id'] = $data['purchase_line_tax_id'] ?? null;

            $new_quantity = $this->num_uf($data['quantity']) * $multiplier;

            // If updating and new quantity is zero, delete the line and adjust stock
            if ($new_quantity == 0) {
                if (isset($data['purchase_line_id'])) {
                    $purchase_line = PurchaseLine::find($data['purchase_line_id']);
                    if ($purchase_line) {
                        // Adjust stock if previous status was received
                        ...
                        // Adjust purchase order/requisition lines if needed
                        if (!empty($purchase_line->purchase_order_line_id)) {
                            $this->updatePurchaseOrderLine($purchase_line->purchase_order_line_id, 0, $purchase_line->quantity);
                        }
                        if (!empty($purchase_line->purchase_requisition_line_id)) {
                            $this->updatePurchaseOrderLine($purchase_line->purchase_requisition_line_id, 0, $purchase_line->quantity);
                        }
                        $purchase_line->delete();
                    }
                }
                // For new lines, just skip
                continue;
            }
            ...
            $purchase_line->row_discount_type = !empty($data['row_discount_type']) ? $data['row_discount_type'] : 'fixed';
        ...
        foreach ($transaction->purchase_lines as $purchase_line) {
            if ($purchase_line->product->enable_stock == 1) {
                ...
                $sell_line_purchase_lines =
                    ...
                    ->get();
                //
                $details = VariationLocationDetails::where('variation_id', $purchase_line->variation_id)
                    ->where('product_id', $purchase_line->product_id)
                    ->where('location_id', $transaction->location_id)
                    ->first();

                $just = $details->in_stock_qty - $purchase_line_qty_avlbl;
                //
                ...
                //
                if ($just <= 0) { // just mean before transaction 
                    $remaining = TransactionSellLine::with(['transaction'])
                        ->whereHas('transaction', function ($query) use ($transaction) {
                            $query->where('type', 'sales_order')
                                ->where('status', 'ordered')
                    ...
                    // old logic 
                    // $add_in_stock_qty =abs( $remaining - abs($details->qty_available));
                    // if ($add_in_stock_qty <= 0) {
                    //     $details->in_stock_qty = 0;
                    // } else {
                    //     $details->in_stock_qty = $add_in_stock_qty;  // to manage for web 
                    // }

                    // new logic 
                    // For overselling case, calculate virtual stock correctly
                    if ($details->qty_available < 0) {
                        // When hand stock is negative, virtual stock should be 0 until hand stock becomes positive
                        $details->in_stock_qty = 0;
                    } else {
                        // When hand stock is positive, check if we just recovered from overselling
                        // If hand stock was negative before this purchase, virtual should equal hand stock
                        $previous_hand_stock = $details->qty_available - $purchase_line_qty_avlbl;
                        
                        if ($previous_hand_stock < 0) {
                            // We just recovered from overselling, virtual should equal hand stock
                            $details->in_stock_qty = max(0, $details->qty_available);
                        } else {
                            // Normal case, calculate virtual stock as available minus remaining orders
                            $add_in_stock_qty = max(0, $details->qty_available - $remaining);
                            $details->in_stock_qty = $add_in_stock_qty;
                        }
                    }
                    
                } else if ($just > 0) {
                    // $details->in_stock_qty += $purchase_line_qty_avlbl; // already done on create or update purchase line 
                }
                $details->save();
                //
                $purchase_line->save();
    ...
    public function filterProduct($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        ...
        $query->where('products.business_id', $business_id)
            ...
            ->where('products.is_gift_card', 0); // Exclude gift cards from main products list
        ...
        if (! empty($search_term)) {
            ...
            if ($search_type == 'like') {
                $query->where(function ($query) use ($search_term, $search_fields) {
                    ...
                    // if (in_array('barcode_no', $search_fields)) {
                    //     $query->orWhere('barcode_no', 'like', '%'.$search_term.'%');
                    // }
                    if (in_array('var_barcode_no', $search_fields)) {
                        $query->orWhere('var_barcode_no', 'like', '%' . $search_term . '%');
                    }
        ...
        //Include check for quantity - only filter products with stock enabled
        if ($check_qty) {
            $query->where(function($q) {
                $q->where('VLD.qty_available', '>', 0)
    ...
    public function filtermetrix($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        ...
        $query->where('products.business_id', $business_id)
            ->where('products.type', '!=', 'modifier')
            ->where('products.is_gift_card', 0); // Exclude gift cards from metrix products list
        ...
        return $query->select(['name', 'sku', 'id'])->get();
    ...
    public function getProductStockDetails($business_id, $filters, $for)
    {
        ...
        $products = $query->select(
            ...
            DB::raw('SUM(vld.in_stock_qty) as webstock'),
    ...
    public function getProductStockDetailsModelPopup($business_id, $filters, $for)
    {
        $query = Variation::with(['media'])
            ...
            ->leftjoin('categories as c', 'p.category_id', '=', 'c.id')
            ->join('product_variations as pv', 'variations.product_variation_id', '=', 'pv.id')
            ->leftJoin('variation_group_prices as vgp', 'vgp.variation_id', '=', 'variations.id') // Left join for group prices
            ->leftJoin('selling_price_groups as spg', 'vgp.price_group_id', '=', 'spg.id') // Left join for group prices
            ->where('p.business_id', $business_id)
            ->whereIn('p.type', ['single', 'variable'])
            ->where('variations.discontinued', false); // Exclude discontinued variations
        ...
        if ($permitted_locations != 'all') {
            $query->where(function ($q) use ($permitted_locations) {
                $q->whereIn('vld.location_id', $permitted_locations)
                  ->orWhereNull('vld.location_id'); // keep variations even if no location record
            });
        ...
        if ($for == 'view_product') {
            $products = $query->select(
                ...
                DB::raw("(SELECT SUM( COALESCE(pl.quantity - ($pl_query_string), 0) * purchase_price_inc_tax) FROM transactions 
                      ...
                      WHERE (transactions.status='received' OR transactions.type='purchase_return')  AND transactions.location_id=vld.location_id 
                      AND (pl.variation_id=variations.id)) as stock_price"),
                DB::raw('MAX(vld.qty_available) as stock'),
                DB::raw('MAX(vld.in_stock_qty) as webstock'),
                'variations.sub_sku as sku',
                'variations.var_barcode_no as barcode',
                'p.name as product',
                'p.type',
                'p.alert_quantity',
                'p.id as product_id',
                ...
                'pv.name as product_variation',
                'variations.name as variation_name',
                'variations.dpp_inc_tax as cost',
                'l.name as location_name',
                'l.id as location_id',
                'variations.id as variation_id',
                'c.name as category_name',
                ...
                'p.product_custom_field4',
                'variations.*',
                DB::raw("CONCAT('" . asset('/uploads/img/') . "/', p.image) as parent_product_image"),
                // Aggregating group prices as a JSON array in one row
                DB::raw("IFNULL(GROUP_CONCAT(CONCAT_WS(':', spg.name, vgp.price_inc_tax) SEPARATOR ','), '') as group_prices")
            )->groupBy('variations.id', 'vld.location_id');
        } else {
            $products = $query->select(
                // DB::raw("(SELECT SUM(quantity) FROM transaction_sell_lines LEFT JOIN transactions ON transaction_sell_lines.transaction_id=transactions.id WHERE transactions.status='final' $location_filter AND
    ...
    public function fixVariationStockMisMatch($business_id, $variation_id, $location_id, $stock)
    {
        ...
        if (! empty($vld)) {
            // Track old stock before updating
            $old_stock = $vld->qty_available;
            $old_in_stock = $vld->in_stock_qty;
            
            
            $vld->qty_available = $stock;
            
            // Calculate virtual stock (in_stock_qty) based on hand stock and remaining orders
            if ($vld->qty_available < 0) {
                $vld->in_stock_qty = 0;
            } else {
                // Calculate remaining ordered quantity
                $remaining_ordered = DB::table('transaction_sell_lines as tsl')
                    ->join('transactions as t', 'tsl.transaction_id', '=', 't.id')
                    ->where('t.type', 'sales_order')
                    ->where('t.status', '!=', 'cancelled')
                ...
                $vld->in_stock_qty = max(0, $vld->qty_available - $remaining_ordered);
            }
            
            $vld->save();
            
            // Check if stock was increased and product is now in stock
            $stock_increase = $stock - $old_stock;
            // Trigger alert if stock increased from 0 or negative to positive
            if ($stock_increase > 0 && $old_stock <= 0 && $vld->qty_available > 0) {
                // Trigger stock alert
                $this->triggerStockAlert($vld->product_id, $variation_id, $business_id);
            }
        }
    ...
    public function updateComboProductStock($variation_id, $location_id)
    {
        $combo_variations = Variation::where('combo_variations', 'like', '%"variation_id":' . $variation_id . '%')
                                    ->orWhere('combo_variations', 'like', '%"variation_id":"' . $variation_id . '"%')
                                    ->get();

        foreach ($combo_variations as $combo_v) {
            $product = Product::find($combo_v->product_id);
            if (!empty($product) && $product->type == 'combo') {
                $qty_available = $this->calculateComboQuantity($location_id, $combo_v->combo_variations);
                
                VariationLocationDetails::updateOrCreate(
                    ['variation_id' => $combo_v->id, 'location_id' => $location_id],
                    [
                        'product_id' => $combo_v->product_id,
                        'product_variation_id' => $combo_v->product_variation_id,
