...
class Util
{
     ...
     * @return int
     */
    public function generateReferenceNumber($type, $ref_count, $business_id = null, $default_prefix = null, $is_ecom=false)
    {
        $prefix = '';
        if($is_ecom){
     ...
     *
     * @param  object  $request, string $file_name, string dir_name , string $filePrefix = null
     * @return string
     */
    public function uploadFile($request, $file_name, $dir_name, $file_type = 'document', $filePrefix = null)
    {
        ...
        if ($request->hasFile($file_name) && $request->file($file_name)->isValid()) {
            ...
            if ($request->$file_name->getSize() <= config('constants.document_size_limit')) {
                ...
                $originalName = $request->$file_name->getClientOriginalName();
    ...
    public function uploadFavicon($request, $file_name, $file_type = 'document')
    {
        $public_path = public_path(); // Points to the public folder
        ...
        if ($request->hasFile($file_name) && $request->file($file_name)->isValid()) {
            ...
            if ($request->$file_name->getSize() <= config('constants.document_size_limit')) {
                ...
                $destination_path = $public_path.'/uploads/business_logos/' . DIRECTORY_SEPARATOR . $new_file_name;
                if (!file_exists($destination_path)) {
                    mkdir($destination_path, 0775, true);
                }
                if ($request->$file_name->move($destination_path, $new_file_name)) {
                    $uploaded_file_name = $new_file_name;
                }
            }
        }
    ...
    public function uploadBrandLogo($request, $file_name, $dir_name, $file_type = 'document')
    {
    ...
    public function replaceTags($business_id, $data, $transaction, $contact = null)
    {
        ...
        // For sales orders, try to get shipment data from related invoice
        $shipmentTransaction = $transaction;
        if ($transaction->type === 'sales_order') {
            ...
            if ($relatedInvoice && !empty($relatedInvoice->shipment)) {
                $shipmentTransaction = $relatedInvoice;
            }
        }
        ...
        foreach ($data as $key => $value) {
            ...
            if (strpos($value, '{invoice_number}') !== false) {
                $invoice_number = '';
                if ($transaction->type == 'sell' || $transaction->type == 'sell_return_ecom' || $transaction->type == 'sales_order') {
            ...
            if (strpos($value, '{business_logo}') !== false) {
                ...
                $business_logo = ! empty($logo_name) ? '<img src="'.url('uploads/business_logos/'.$logo_name).'"  style="max-width: 300px; height: auto;" alt="Business Logo" >' : '';

                $data[$key] = str_replace('{business_logo}', $business_logo, $data[$key]);
            }

            //Replace invoice_url
            if (! empty($transaction) && strpos($value, '{invoice_url}') !== false && ($transaction->type == 'sell' || $transaction->type == 'sell_return_ecom')) {
            ...
            if (strpos($value, '{shipping_custom_field_5}') !== false) {
             ...
             //Replace shipment number
             if (strpos($value, '{shipment_number}') !== false) {
                $shipment_number = '';
                if (($transaction->type == 'sell' || $transaction->type == 'sales_order') && !empty($transaction->shipment)) {
                    $shipment_data = is_string($transaction->shipment) ? json_decode($transaction->shipment, true) : $transaction->shipment;
                    $shipment_number = $shipment_data['packages'][0]['package_id'] ?? "N/A";
                }
                $data[$key] = str_replace('{shipment_number}', $shipment_number, $data[$key]);
            }
            if (strpos($value, '{tracking_number}') !== false) {
                $tracking_number = '';
                if (($transaction->type == 'sell' || $transaction->type == 'sales_order') && !empty($transaction->shipment)) {
                    $shipment_data = is_string($transaction->shipment) ? json_decode($transaction->shipment, true) : $transaction->shipment;
                    $tracking_number = $shipment_data['shipment_details']['tracking_number'] ?? '';
                }
                $data[$key] = str_replace('{tracking_number}', $tracking_number, $data[$key]);
            }
            if (strpos($value, '{carrier_name}') !== false) {
                $carrier_name = '';
                if (($transaction->type == 'sell' || $transaction->type == 'sales_order') && !empty($transaction->shipment)) {
                    $shipment_data = is_string($transaction->shipment) ? json_decode($transaction->shipment, true) : $transaction->shipment;
                    $carrier_name = $shipment_data['shipment_details']['carrier_name'] ?? '';
                }
                $data[$key] = str_replace('{carrier_name}', $carrier_name, $data[$key]);
            }
            if (strpos($value, '{estimated_delivery}') !== false) {
                $estimated_delivery = '';
                if (($transaction->type == 'sell' || $transaction->type == 'sales_order') && !empty($transaction->shipment)) {
                    $shipment_data = is_string($transaction->shipment) ? json_decode($transaction->shipment, true) : $transaction->shipment;
                    if (isset($shipment_data['shipment_details']['delivery_date'])) {
                        $estimated_delivery = date('d/m/Y', strtotime($shipment_data['shipment_details']['delivery_date']));
                    }
                }
                $data[$key] = str_replace('{estimated_delivery}', $estimated_delivery, $data[$key]);
            }
            ...
            if (strpos($value, '{tracking_url}') !== false) {
                $tracking_url = '';
                if (!empty($shipmentTransaction->shipment)) {
                    $shipment_data = is_string($shipmentTransaction->shipment) ? json_decode($shipmentTransaction->shipment, true) : $shipmentTransaction->shipment;
                    if (is_array($shipment_data) && isset($shipment_data['shipment_details']['tracking_url'])) {
                        $tracking_url = $shipment_data['shipment_details']['tracking_url'];
                    }
                }
                $data[$key] = str_replace('{tracking_url}', $tracking_url, $data[$key]);
            }
    ...
    public function replaceCustomTags($business_id, $data, $custom_data, $contact = null)
    {
        ...
        $contact = $custom_data;
        $brand_id = isset($contact->brand_id) ? $contact->brand_id : null;
        foreach ($data as $key => $value) {
            //Replace business_name
            if (strpos($value, '{business_name}') !== false) {
                $business_name = $business->name;
            ...
            //Replace contact name
            if (strpos($value, '{contact_name}') !== false) {
                // if custommeer then name if App/User then first_name and last_name
                $contact_name =$custom_data->name;
                $data[$key] = str_replace('{contact_name}', $contact_name, $data[$key]);
            }
            //Replace OTP (B2B forgot-password flow)
            if (strpos($value, '{otp}') !== false) {
                $otp = '';
                if (is_object($custom_data) && isset($custom_data->otp)) {
                    $otp = $custom_data->otp;
                } elseif (is_array($custom_data) && isset($custom_data['otp'])) {
                    $otp = $custom_data['otp'];
                }
                $data[$key] = str_replace('{otp}', (string) $otp, $data[$key]);
            }
            // Reset password page URL (frontend page where user enters OTP and new password)
            if (strpos($value, '{reset_password_page_url}') !== false) {
                $default_url = config('app.front-url');
                $base_url = $default_url;
                if (! empty($brand_id)) {
                    $brand = \App\Brands::where('id', $brand_id)->first();
                    $base_url = $brand && $brand->brand_url ? $brand->brand_url : $default_url;
                }
                $email = is_object($custom_data) ? ($custom_data->email ?? '') : ($custom_data['email'] ?? '');
                $reset_url = rtrim($base_url, '/') . '/set-password?email=' . urlencode($email);
                $data[$key] = str_replace('{reset_password_page_url}', $reset_url, $data[$key]);
            }
             //Replace ref no
            if (strpos($value, '{ref_no}') !== false) {
                $contact_ref_no =$custom_data->ref_no;
                $data[$key] = str_replace('{ref_no}', $contact_ref_no, $data[$key]);
            }

            //Replace complete url
            if (strpos($value, '{url_complete}') !== false) {
                $token=$custom_data->remember_token;
                $email=urlencode($custom_data->email);
                $default_url=config('app.front-url');
                $app_url=$default_url;
                $brand_url=null;
                if($brand_id){
                    $brand_url=Brands::where('id', $brand_id)->first()->brand_url;
                    $app_url=$brand_url??$default_url;
                    $app_url=$app_url.'/auth';
                }
                $full_url= $app_url. '/set-password?token=' . $token . '&email=' .$email;
                $data[$key] = str_replace('{url_complete}', $full_url, $data[$key]);
            }

            //Replace complete url
            if (strpos($value, '{url_business}') !== false) {
                $default_url=config('app.front-url');
                $app_url=$default_url;
                $brand_url=null;
                if($brand_id){
                ...
                $data[$key] = str_replace('{url_business}', $app_url, $data[$key]);
            }    
            //Replace email confirmation link
            if (strpos($value, '{email_confirmation_link}') !== false) {
                $token=$custom_data->remember_token;
                $default_url=config('app.front-url');
                $app_url=$default_url;
                $brand_url=null;
                ...
                $final_url=$app_url.'/verify-email?token=' . $token .'&email=' . $custom_data->email;
                $data[$key] = str_replace('{email_confirmation_link}', $final_url, $data[$key]);
            }    
            //Replace brand logo
            if (strpos($value, '{brand_logo}') !== false) {
                $brand_logo_src =Brands::where('id', $custom_data->brand_id)->first()->logo;
                $brand_logo = ! empty($brand_logo_src) ? '<img src="'.url('uploads/img/'.$brand_logo_src).'"  style="max-width: 300px; height: auto;" alt="Brand Logo" >' : '';
                $data[$key] = str_replace('{brand_logo}', $brand_logo, $data[$key]);
            }
            
            // Stock Alert specific tags
            //Replace product_name
            if (strpos($value, '{product_name}') !== false) {
                $product_name = isset($custom_data->product_name) ? $custom_data->product_name : '';
                $data[$key] = str_replace('{product_name}', $product_name, $data[$key]);
            }
            
            //Replace product_sku
            if (strpos($value, '{product_sku}') !== false) {
                $product_sku = isset($custom_data->product_sku) ? $custom_data->product_sku : '';
                $data[$key] = str_replace('{product_sku}', $product_sku, $data[$key]);
            }
            
            //Replace product_url
            if (strpos($value, '{product_url}') !== false) {
                $product_url = isset($custom_data->product_url) ? $custom_data->product_url : '';
                $data[$key] = str_replace('{product_url}', $product_url, $data[$key]);
            }
            
            //Replace unsubscribe_url
            if (strpos($value, '{unsubscribe_url}') !== false) {
                $unsubscribe_url = isset($custom_data->unsubscribe_url) ? $custom_data->unsubscribe_url : '';
                $data[$key] = str_replace('{unsubscribe_url}', $unsubscribe_url, $data[$key]);
            }
            
            // Credit approval specific tags
            //Replace credit_limit
            if (strpos($value, '{credit_limit}') !== false) {
                $credit_limit = isset($custom_data->credit_limit) ? $custom_data->credit_limit : '';
                $data[$key] = str_replace('{credit_limit}', $credit_limit, $data[$key]);
            }
            
            //Replace requested_credit_amount
            if (strpos($value, '{requested_credit_amount}') !== false) {
                $requested_credit_amount = isset($custom_data->requested_credit_amount) ? $custom_data->requested_credit_amount : '';
                $data[$key] = str_replace('{requested_credit_amount}', $requested_credit_amount, $data[$key]);
            }

            // Gift card specific tags
            if (strpos($value, '{gift_card_code}') !== false) {
                $gift_card_code = isset($custom_data->gift_card_code) ? $custom_data->gift_card_code : '';
                $data[$key] = str_replace('{gift_card_code}', $gift_card_code, $data[$key]);
            }

            if (strpos($value, '{gift_card_amount}') !== false) {
                $gift_card_amount = isset($custom_data->gift_card_amount) ? $custom_data->gift_card_amount : '';
                $data[$key] = str_replace('{gift_card_amount}', $gift_card_amount, $data[$key]);
            }

            if (strpos($value, '{gift_card_balance}') !== false) {
                $gift_card_balance = isset($custom_data->gift_card_balance) ? $custom_data->gift_card_balance : '';
                $data[$key] = str_replace('{gift_card_balance}', $gift_card_balance, $data[$key]);
            }

            if (strpos($value, '{gift_card_currency}') !== false) {
                $gift_card_currency = isset($custom_data->gift_card_currency) ? $custom_data->gift_card_currency : '';
                $data[$key] = str_replace('{gift_card_currency}', $gift_card_currency, $data[$key]);
            }

            if (strpos($value, '{gift_card_expires_at}') !== false) {
                $gift_card_expires_at = isset($custom_data->gift_card_expires_at) ? $custom_data->gift_card_expires_at : '';
                $data[$key] = str_replace('{gift_card_expires_at}', $gift_card_expires_at, $data[$key]);
            }

            if (strpos($value, '{gift_card_message}') !== false) {
                $gift_card_message = isset($custom_data->gift_card_message) ? $custom_data->gift_card_message : '';
                $data[$key] = str_replace('{gift_card_message}', $gift_card_message, $data[$key]);
            }
        }
        return $data;
    }

    public function replaceCustomTagsPayments($business_id, $data, $custom_data, $contact = null)
    {
      
        $business = ! is_object($business_id) ? Business::with(['currency'])->findOrFail($business_id) : $business_id;

        $contact = empty($custom_data['contact']) ? $contact : $custom_data['contact'];
        // dd($contact);
        foreach ($data as $key => $value) {
            //Replace business_name
            if (strpos($value, '{business_name}') !== false) {
            ...
            // replace Amount
            if (strpos($value, '{payment_amount}') !== false) {
                $payment_amount =$custom_data['amount'];
                $data[$key] = str_replace('{payment_amount}', $payment_amount, $data[$key]);
            }

            // replace paymentLink
            if (strpos($value, '{payment_link}') !== false) {
                $front_end_url = config('app.url');
                $payment_link = $front_end_url . '/payment-request/'. '?token=' .$custom_data['token']. '&id=' . $custom_data['customer_id'];
                $data[$key] = str_replace('{payment_link}', $payment_link, $data[$key]);
            }
             //Replace credit_limit
             if (strpos($value, '{credit_limit}') !== false) {
             ...
             if (strpos($value, '{requested_credit_amount}') !== false) {
                 $requested_credit_amount = $contact->requested_credit_amount ?? '';
                 $data[$key] = str_replace('{requested_credit_amount}', $requested_credit_amount, $data[$key]);
             }
    ...
    /**
     * Retrieves net due amount of a contact (customer + supplier)
     *
     * Logic:
     * - Sales (sell) & opening balance are treated as amounts the contact owes to the business
     * - Purchases are treated as amounts the business owes to the contact
     * - Result is a *net* balance:
     *     > 0  => contact owes business (receivable)
     *     < 0  => business owes contact (payable)
     *
    ...
    public function getContactDue($contact_id, $business_id = null)
    {
        ...
        // Net due = (customer side: invoices + opening_balance - payments_received)
        //           minus (supplier side: purchases - payments_made)
        ...
        // For customers, treat available credit (contact.balance) as reducing net due.
        // This ensures that top-ups recorded as customer balance/Open Balance
        // visibly reduce the "Balance due" / "Overdue Payment" header.
        $contact = Contact::find($contact_id);
        if ($contact && in_array($contact->type, ['customer', 'both'])) {
            $due -= (float) $contact->balance;
        }
    ...
    public function parseNotifications($notifications){
        ...
        foreach ($notifications as $notification) {
            ...
            // dd($data['data']['subject'],$data['data']['sms_body'],$data['data']['email_body'],$data['data']['whatsapp_text']);
            $parsed[] = [
                'id' => $notification->id,
                'title' => $data['data']['subject'] ?? $data['data']['title'] ?? 'Notification',
                'body' => $data['data']['sms_body'] ?? $data['data']['email_body'] ?? $data['data']['whatsapp_text'] ??  '',
    ...
    /**
     * Get the product visibility options
     *
     * @return array
     */
    ...
    /**
     * Replaces brand-specific tags from notification body with original values
     *
     * @param  int  $business_id
     * @param  int  $brand_id
     * @param  array  $data
     ...
     * @return array
     */
    public function replaceBrandTags($business_id, $brand_id, $data, $custom_data = null, $contact = null, $transaction = null)
    {
        $business = Business::with(['currency'])->findOrFail($business_id);
        $brand = Brands::findOrFail($brand_id);
        $user = null;
        if (isset($custom_data['user']->contact_id)) {
            // Try to resolve by external contact_id first, then by primary key, but don't hard-fail
            $user = Contact::where('contact_id', $custom_data['user']->contact_id)->first();
            if (! $user) {
                $user = Contact::find($custom_data['user']->contact_id);
            }
        } elseif (isset($custom_data['contact_id'])) {
            $user = Contact::find($custom_data['contact_id']);
        } else {
            $user = $custom_data['user'] ?? null;
        }

        // Convert custom_data to array if it's an object
        ...
        foreach ($data as $key => $value) {
            ...
            if (strpos($value, '{brand_logo}') !== false) {
                ...
                $brand_logo = !empty($brand_logo_name) ? '<img src="'.url('uploads/img/'.$brand_logo_name).'" style="max-width: 300px; height: auto;" alt="Brand Logo">' : '';
                $data[$key] = str_replace('{brand_logo}', $brand_logo, $data[$key]);
            }

            // Replace brand_name
            if (strpos($value, '{brand_name}') !== false) {
                $brand_name = $brand->name ?? '';
                $data[$key] = str_replace('{brand_name}', $brand_name, $data[$key]);
            }

            // Replace contact_name (from contact object or user)
            if (strpos($value, '{contact_name}') !== false) {
                $user_name = $user?->name ?? '';
                $data[$key] = str_replace('{contact_name}', $user_name, $data[$key]);
            }

            // Replace url_business / front_url
            if (strpos($value, '{url_business}') !== false) {
                $url_business = !empty($brand->brand_url) ? $brand->brand_url : url('/');
                $data[$key] = str_replace('{url_business}', $url_business, $data[$key]);
            }
            
            if (strpos($value, '{front_url}') !== false) {
                $front_url = !empty($brand->brand_url) ? $brand->brand_url : url('/');
                $data[$key] = str_replace('{front_url}', $front_url, $data[$key]);
            }

            // Replace brand_ecom_login_url
            if (strpos($value, '{brand_ecom_login_url}') !== false) {
                $brand_ecom_login_url = $brand->brand_url . '/auth/login';
                $data[$key] = str_replace('{brand_ecom_login_url}', $brand_ecom_login_url, $data[$key]);
            }

            // Replace verification_link (for password reset)
            if (strpos($value, '{verification_link}') !== false) {
                $token = $user->remember_token ?? '';
                $email = urlencode($user->email ?? '');
                $verification_link = $brand->brand_url. '/auth/set-password?token=' . $token . '&email=' .$email;
                $data[$key] = str_replace('{verification_link}', $verification_link, $data[$key]);
            }

            // Replace email_confirmation_link
            if (strpos($value, '{email_confirmation_link}') !== false) {
                $email_confirmation_link = '';
                if (isset($custom_data['user'])) {
                    $user = is_array($custom_data['user']) ? (object)$custom_data['user'] : $custom_data['user'];
                    $app_url = !empty($brand->brand_url) ? $brand->brand_url : url('/');
                    $token = $user->remember_token ?? '';
                    $email = urlencode($user->email ?? '');
                    $email_confirmation_link = $app_url . '/verify-email?token=' . $token . '&email=' . $email;
                }
                $data[$key] = str_replace('{email_confirmation_link}', $email_confirmation_link, $data[$key]);
            }

            // Replace url_complete (for set password)
            if (strpos($value, '{url_complete}') !== false) {
                $url_complete = '';
                if (isset($custom_data['user'])) {
                    $user = is_array($custom_data['user']) ? (object)$custom_data['user'] : $custom_data['user'];
                    ...
                    $token = $user->remember_token ?? '';
                    $email = urlencode($user->email ?? '');
                    $url_complete = $app_url . '/auth/set-password?token=' . $token . '&email=' . $email;
                }
                $data[$key] = str_replace('{url_complete}', $url_complete, $data[$key]);
            }

            // Replace ref_no
            if (strpos($value, '{ref_no}') !== false) {
                $ref_no = isset($custom_data['user']->ref_no) ? $custom_data['user']->ref_no : '';
                $data[$key] = str_replace('{ref_no}', $ref_no, $data[$key]);
            }

            // Replace unsubscribe_url
            if (strpos($value, '{unsubscribe_url}') !== false) {
                $unsubscribe_url = $brand->brand_url . '/unsubscribe?email=' . urlencode($user->email);
                $data[$key] = str_replace('{unsubscribe_url}', $unsubscribe_url, $data[$key]);
            }
            if(strpos($value, '{cupon_code}') !== false){
                $cupon_code = isset($custom_data['cupon_code']) ? $custom_data['cupon_code'] : '';
                $data[$key] = str_replace('{cupon_code}', $cupon_code, $data[$key]);
            }

            // Replace invoice-related tags if transaction data is in custom_data
            if (!empty($transaction)) {
                
                // Replace invoice number
                if (strpos($value, '{invoice_number}') !== false) {
                    $invoice_number = is_object($transaction) ? ($transaction->invoice_no ?? '') : ($transaction['invoice_no'] ?? '');
                    $data[$key] = str_replace('{invoice_number}', $invoice_number, $data[$key]);
                }

                // Replace order_ref_number
                if (strpos($value, '{order_ref_number}') !== false) {
                    $order_ref_number = is_object($transaction) ? ($transaction->ref_no ?? '') : ($transaction['ref_no'] ?? '');
                    $data[$key] = str_replace('{order_ref_number}', $order_ref_number, $data[$key]);
                }

                // Replace invoice_url
                if (strpos($value, '{invoice_url}') !== false) {
                    $transaction_id = is_object($transaction) ? $transaction->id : ($transaction['id'] ?? null);
                    $transaction_business_id = is_object($transaction) ? $transaction->business_id : ($transaction['business_id'] ?? $business_id);
                    if (!empty($transaction_id)) {
                        $invoice_url = $this->getInvoiceUrl($transaction_id, $transaction_business_id);
                        $data[$key] = str_replace('{invoice_url}', $invoice_url, $data[$key]);
                    }
                }
                ...
                // Replace total_amount
                if (strpos($value, '{total_amount}') !== false) {
                    $final_total = is_object($transaction) ? ($transaction->final_total ?? 0) : ($transaction['final_total'] ?? 0);
                    $total_amount = $this->num_f($final_total, true, $business);
                    $data[$key] = str_replace('{total_amount}', $total_amount, $data[$key]);
                }

                // Replace order_date
                if (strpos($value, '{order_date}') !== false) {
                    $transaction_date = is_object($transaction) ? ($transaction->transaction_date ?? null) : ($transaction['transaction_date'] ?? null);
                    if (!empty($transaction_date)) {
                        $order_date = $this->format_date($transaction_date, true, $business);
                        $data[$key] = str_replace('{order_date}', $order_date, $data[$key]);
                    }
                }

                // Replace shipping_address
                if (strpos($value, '{shipping_address}') !== false) {
                    $shipping_address = is_object($transaction) ? ($transaction->shipping_address ?? '') : ($transaction['shipping_address'] ?? '');
                    $data[$key] = str_replace('{shipping_address}', $shipping_address, $data[$key]);
                }

                // Replace billing_address
                if (strpos($value, '{billing_address}') !== false) {
                    $billing_address = '';
                    if (is_object($transaction)) {
                        $address_parts = [];
                        // Try billing fields first
                        if (!empty($transaction->billing_address1 ?? null)) $address_parts[] = $transaction->billing_address1;
                        if (!empty($transaction->billing_address2 ?? null)) $address_parts[] = $transaction->billing_address2;
                        if (!empty($transaction->billing_city ?? null)) $address_parts[] = $transaction->billing_city;
                        if (!empty($transaction->billing_state ?? null)) $address_parts[] = $transaction->billing_state;
                        if (!empty($transaction->billing_country ?? null)) $address_parts[] = $transaction->billing_country;
                        if (!empty($transaction->billing_zip ?? null)) $address_parts[] = $transaction->billing_zip;
                        
                        // If billing address is empty, fall back to shipping fields
                        if (empty($address_parts)) {
                            if (!empty($transaction->shipping_address1 ?? null)) $address_parts[] = $transaction->shipping_address1;
                            if (!empty($transaction->shipping_address2 ?? null)) $address_parts[] = $transaction->shipping_address2;
                            if (!empty($transaction->shipping_city ?? null)) $address_parts[] = $transaction->shipping_city;
                            if (!empty($transaction->shipping_state ?? null)) $address_parts[] = $transaction->shipping_state;
                            if (!empty($transaction->shipping_country ?? null)) $address_parts[] = $transaction->shipping_country;
                            if (!empty($transaction->shipping_zip ?? null)) $address_parts[] = $transaction->shipping_zip;
                        }
                    ...
                    $data[$key] = str_replace('{billing_address}', $billing_address, $data[$key]);
                }

                // Replace pickup_address
                if (strpos($value, '{pickup_address}') !== false) {
                    $pickup_address = isset($custom_data['pickup_address']) ? $custom_data['pickup_address'] : '';
                    if (empty($pickup_address) && is_object($transaction) && !empty($transaction->location)) {
                        $pickup_address = $transaction->location->location_address ?? '';
                    }
                    $data[$key] = str_replace('{pickup_address}', $pickup_address, $data[$key]);
                }

                // Calculate paid and due amounts
                $total_paid = 0;
                $payment_lines = is_object($transaction) ? ($transaction->payment_lines ?? []) : ($transaction['payment_lines'] ?? []);
                if (!empty($payment_lines)) {
                    foreach ($payment_lines as $payment) {
                        $is_return = is_object($payment) ? ($payment->is_return ?? 0) : ($payment['is_return'] ?? 0);
                ...
                // Replace paid_amount
                if (strpos($value, '{paid_amount}') !== false) {
                    $paid_amount = $this->num_f($total_paid, true, $business);
                    $data[$key] = str_replace('{paid_amount}', $paid_amount, $data[$key]);
                }
                ...
                if (strpos($value, '{due_amount}') !== false) {
                    ...
                    $due_amount = $this->num_f($due, true, $business);
                    $data[$key] = str_replace('{due_amount}', $due_amount, $data[$key]);
                }

                // Replace cumulative_due_amount
                if (strpos($value, '{cumulative_due_amount}') !== false) {
                    $contact_id = is_object($transaction) ? ($transaction->contact_id ?? null) : ($transaction['contact_id'] ?? null);
                    if (!empty($contact_id)) {
                        $cumulative_due = $this->getContactDue($contact_id, $business_id);
                        $data[$key] = str_replace('{cumulative_due_amount}', $cumulative_due, $data[$key]);
                    }
                }

                // Replace due_date
                if (strpos($value, '{due_date}') !== false) {
                    $due_date = is_object($transaction) ? ($transaction->due_date ?? null) : ($transaction['due_date'] ?? null);
                    if (!empty($due_date)) {
                        $due_date_formatted = $this->format_date($due_date, true, $business);
                        $data[$key] = str_replace('{due_date}', $due_date_formatted, $data[$key]);
                    }
                }

                // Replace location tags if location is available
                $location = is_object($transaction) ? ($transaction->location ?? null) : ($transaction['location'] ?? null);
                if (!empty($location)) {
                    if (strpos($value, '{location_name}') !== false) {
                        $location_name = is_object($location) ? ($location->name ?? '') : ($location['name'] ?? '');
                        $data[$key] = str_replace('{location_name}', $location_name, $data[$key]);
                    }
                    if (strpos($value, '{location_address}') !== false) {
                        $location_address = is_object($location) ? ($location->location_address ?? '') : ($location['location_address'] ?? '');
                        $data[$key] = str_replace('{location_address}', $location_address, $data[$key]);
                    }
                    if (strpos($value, '{location_email}') !== false) {
                        $location_email = is_object($location) ? ($location->email ?? '') : ($location['email'] ?? '');
                        $data[$key] = str_replace('{location_email}', $location_email, $data[$key]);
                    }
                    if (strpos($value, '{location_phone}') !== false) {
                        $location_phone = is_object($location) ? ($location->mobile ?? '') : ($location['mobile'] ?? '');
                        $data[$key] = str_replace('{location_phone}', $location_phone, $data[$key]);
                    }
                }
            }

            // Replace product-related tags if product data is in custom_data
            if (!empty($custom_data) && is_array($custom_data) && isset($custom_data['product'])) {
                $product = $custom_data['product'];
                ...
                if (strpos($value, '{product_sku}') !== false) {
                    $product_sku = is_object($product) ? ($product->sku ?? '') : ($product['sku'] ?? '');
                    $data[$key] = str_replace('{product_sku}', $product_sku, $data[$key]);
                }
                if (strpos($value, '{product_url}') !== false) {
                    $product_id = is_object($product) ? ($product->id ?? null) : ($product['id'] ?? null);
                    if (!empty($product_id)) {
            ...
            // Replace shipment/order status tags if available
            if (!empty($custom_data) && is_array($custom_data)) {
                if (!empty($custom_data['shipment_number']) && strpos($value, '{shipment_number}') !== false) {
                    $data[$key] = str_replace('{shipment_number}', $custom_data['shipment_number'], $data[$key]);
                }
                if (!empty($custom_data['carrier_name']) && strpos($value, '{carrier_name}') !== false) {
                    $data[$key] = str_replace('{carrier_name}', $custom_data['carrier_name'], $data[$key]);
                }
                if (!empty($custom_data['tracking_number']) && strpos($value, '{tracking_number}') !== false) {
                    $data[$key] = str_replace('{tracking_number}', $custom_data['tracking_number'], $data[$key]);
                }
                if (!empty($custom_data['tracking_url']) && strpos($value, '{tracking_url}') !== false) {
                    $data[$key] = str_replace('{tracking_url}', $custom_data['tracking_url'], $data[$key]);
                }
                if (!empty($custom_data['estimated_delivery']) && strpos($value, '{estimated_delivery}') !== false) {
                    $data[$key] = str_replace('{estimated_delivery}', $custom_data['estimated_delivery'], $data[$key]);
                }
                if (!empty($custom_data['packed_date']) && strpos($value, '{packed_date}') !== false) {
                    $data[$key] = str_replace('{packed_date}', now()->format('d/m/Y'), $data[$key]);
                }
            }

            // Replace credit limit and requested credit amount tags
            if (strpos($value, '{credit_limit}') !== false) {
                $credit_limit = '';
                if (!empty($contact) && isset($contact->credit_limit)) {
                    $credit_limit = $contact->credit_limit;
                } elseif (isset($custom_data['credit_limit'])) {
                    $credit_limit = $custom_data['credit_limit'];
                }
                $data[$key] = str_replace('{credit_limit}', $credit_limit, $data[$key]);
            }
            ...
            if (strpos($value, '{requested_credit_amount}') !== false) {
                ...
                if (!empty($contact) && isset($contact->requested_credit_amount)) {
                    $requested_credit_amount = $contact->requested_credit_amount;
                } elseif (isset($custom_data['requested_credit_amount'])) {
                    $requested_credit_amount = $custom_data['requested_credit_amount'];
                }
                $data[$key] = str_replace('{requested_credit_amount}', $requested_credit_amount, $data[$key]);
            }
            ...
            if (strpos($value, '{payment_link}') !== false) {
                $payment_link = isset($custom_data['payment_link']) ? $custom_data['payment_link'] : '';
                $data[$key] = str_replace('{payment_link}', $payment_link, $data[$key]);
            }

            // Replace payment amount
            if (strpos($value, '{payment_amount}') !== false) {
                $payment_amount = isset($custom_data['payment_amount']) ? $custom_data['payment_amount'] : '';
                $data[$key] = str_replace('{payment_amount}', $payment_amount, $data[$key]);
            }
        }
