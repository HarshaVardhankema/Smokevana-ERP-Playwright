...
class TransactionUtil extends Util
{
    ...
    public function updateSellTransaction($transaction_id, $business_id, $input, $invoice_total, $user_id, $uf_data = true, $change_invoice_number = true)
    {
        ...
        $update_date = [
            ...
            'is_kitchen_order' => ! empty($input['is_kitchen_order']) ? 1 : 0,
            //shipping address
            'shipping_first_name' => $input['shipping_first_name'] ?? $transaction->shipping_first_name,
            'shipping_last_name' => $input['shipping_last_name'] ?? $transaction->shipping_last_name,
            'shipping_company' => $input['shipping_company'] ?? $transaction->shipping_company,
            'shipping_address1' => $input['shipping_address1'] ?? $transaction->shipping_address1,
            'shipping_address2' => $input['shipping_address2'] ?? $transaction->shipping_address2,
            'shipping_city' => $input['shipping_city'] ?? $transaction->shipping_city,
            'shipping_state' => $input['shipping_state'] ?? $transaction->shipping_state,
            'shipping_zip' => $input['shipping_zip'] ?? $transaction->shipping_zip,
            'shipping_country' => $input['shipping_country'] ?? $transaction->shipping_country ?? 'US',
        ];
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        foreach ($products as $product) {
            ...
            if (! empty($product['transaction_sell_lines_id'])) {
                ...
                $line = [
                    ...
                    'sub_sku' => $product['sub_sku'] ?? null,
                    'quantity' => $uf_quantity * $multiplier,
                    'ordered_quantity' => $uf_quantity * $multiplier,
                    // for SO only if status is picked or picking
                    'picked_quantity' => ($transaction->picking_status == 'PICKED' || $transaction->picking_status == 'PICKING') && $transaction->picking_started_at != null ? $uf_quantity * $multiplier : 0,
                    'verified_qty' => $transaction->picking_status == 'PICKED' && $transaction->picking_started_at != null ? $uf_quantity * $multiplier : 0,
                    'manual_picked_qty' => ($transaction->picking_status == 'PICKED' || $transaction->picking_status == 'PICKING') && $transaction->picking_started_at != null ? $uf_quantity * $multiplier : 0,
                    //  end of SO only
        ...
        if (! empty($edit_ids)) {
            ...
            if ($transaction->type == 'sales_order') {
                // no need to adjust quantity , logic already at app/Utils/ProductUtil.php adjustProductStockForInvoice
                $this->deleteSellLines($deleted_lines, $location_id, false);
            } else {
        ...
        if (! empty($lines_formatted)) {
            ...
            // Increment top_selling count for each product when creating a new sales_order
            if ($transaction->type == 'sales_order' && $transaction->status == 'ordered') {
                $productIds = collect($lines_formatted)->pluck('product_id')->unique();
    ...
    public function editSellLine($product, $location_id, $status_before, $multiplier = 1, $uf_data = true)
    {
        ...
        $variation_name = $product['sub_sku'] ?? DB::select('SELECT sub_sku FROM variations WHERE id = ?', [$product['variation_id']]) ? DB::select('SELECT sub_sku FROM variations WHERE id = ?', [$product['variation_id']])[0]->sub_sku : 'line_id: '.$product['transaction_sell_lines_id'];
        
        $sell_line = TransactionSellLine::with(['product', 'warranties'])
            ->find($product['transaction_sell_lines_id']);

        $old_qty = $sell_line->quantity;
        $old_subtotal = $sell_line->unit_price_inc_tax * $old_qty;
        $new_subtotal = $product['unit_price_inc_tax'] * $product['quantity'];
        ...
        if ($status_before != 'draft') {
            ...
            // Log quantity change activity for sell transactions with final status
            if ($sell_line->transaction && $sell_line->transaction->type == 'sell' && $sell_line->transaction->status == 'final') {
                $this->logSellLineQuantityChange($sell_line, $old_qty, $new_qty, $variation_name ,'quantity');
            }
            // Log price change activity for sell transaction with final status 
            if($sell_line->transaction && $sell_line->transaction->type == 'sell' && $sell_line->transaction->status == 'final') {
                $this->logSellLineQuantityChange($sell_line, $old_subtotal, $new_subtotal, $variation_name, 'price');
            }
        }
        ...
        $sell_line->fill([
            ...
            'ordered_quantity' => $uf_data ? $this->num_uf($product['quantity']) * $multiplier : $product['quantity'] * $multiplier,
            'unit_price_before_discount' => $unit_price_before_discount,
            'unit_price' => $unit_price,
            'line_discount_type' => ! empty($product['line_discount_type']) ? $product['line_discount_type'] : null,
            'line_discount_amount' => $line_discount_amount,
    ...
    /**
     * Log quantity change activity for sell line items
     *
     ...
     * @return void
     */
    private function logSellLineQuantityChange($sell_line, $old_qty, $new_qty, $ref_name, $midname = ' updated to ')
    {
        if ($old_qty != $new_qty) {
            activity()
                ->performedOn($sell_line->transaction)
                ->causedBy(auth()->user())
                ->withProperties([
                    'customized' => [
                        'old_value' => $old_qty,
                        'new_value' => $new_qty,
                        'mid' => $midname,
                        'modal_ref_no' => $ref_name,
                    ]
                ])
                ->useLog('sell_line_quantity_updated')
                ->tap(function($activity) {
                    $activity->business_id = request()->session()->get('user.business_id');
                    $activity->save();
                })
                ->log('sell_line_quantity_updated');
        }
    }
        ...
        if ($increment_qty != 0) {
            ...
            if ($enable_stock == 1) {
                ...
                $details = VariationLocationDetails::where('variation_id', $variation_id)
                    ...
                    ->first();
                if ($details) {
                    $details->qty_available += $increment_qty;
                    // dd('$details->qty_available:'.$details->qty_available, '$increment_qty:'.$increment_qty, '$details->in_stock_qty:'.$details->in_stock_qty);
                    // dd($increment_qty); 
                    if ($details->qty_available <= 0) {
                        ...
                        $add_in_stock_qty = abs($increment_qty) - $remaining;
                        if ($remaining == 0) {
                            $details->in_stock_qty = max($details->qty_available, 0);
                        } else if ($add_in_stock_qty <= 0) {
                            $details->in_stock_qty = 0;
                        } else {
                            $details->in_stock_qty = max($add_in_stock_qty, 0);
                        }
                    } else {
                        $remaining = TransactionSellLine::with(['transaction'])
                            ->whereHas('transaction', function ($query) use ($location_id) {
                        ...
                        if ($increment_qty > 0) {
                            $details->in_stock_qty = max($details->qty_available - $remaining, 0);
                        } else {
                            $details->in_stock_qty = max($details->qty_available, 0);
                        }
                    }
                    $details->save();
                    // dd('$details->qty_available:'.$details->qty_available, '$details->in_stock_qty:'.$details->in_stock_qty, '$increment_qty:'.$increment_qty, '$upqty:'.$increment_qty, '$remaining:'.$remaining );
                }
            }
        }
    }
            ...
            if (! empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    $payment_data = [
                        ...
                        // 'is_return' => isset($payment['is_return']) ? $payment['is_return'] : ($transaction->type == 'sell_return' ? 1 : 0),
    ...
    public function getContactReturnDue($contact_id)
    {
        $result = DB::table('transactions as t')
            ->leftJoin('transaction_payments as tp', 't.id', '=', 'tp.transaction_id')
            ->where('t.contact_id', $contact_id)
            ->where('t.type', 'sell_return')
            ->selectRaw('SUM(t.final_total) as total_return_amount, COALESCE(SUM(tp.amount), 0) as total_paid_amount')
            ->first();

        return ($result->total_return_amount ?? 0) - ($result->total_paid_amount ?? 0);
    }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if (!empty($transaction->sales_person)) {
            $rep = $transaction->sales_person;
            $output['created_by'] = trim(($rep->surname ?? '') . ' ' . ($rep->first_name ?? '') . ' ' . ($rep->last_name ?? ''));
        }
        ...
        $customer = Contact::with('haveSelesRep')->find($transaction->contact_id);
        $output['sell_representative'] = '';
        if (!empty($customer?->haveSelesRep[0])) {
            $rep = $customer->haveSelesRep[0];
            $output['sell_representative'] = trim(($rep->surname ?? '') . ' ' . ($rep->first_name ?? '') . ' ' . ($rep->last_name ?? ''));
        }
        ...
        // Check if location is B2C
        $is_b2c_location = !empty($location_details) && isset($location_details->is_b2c) && $location_details->is_b2c == 1;
        ...
        if ($il->show_customer == 1) {
            ...
            if ($receipt_printer_type != 'printer') {
                ...
                // Hide mobile and email for B2C locations
                if (!$is_b2c_location) {
        ...
        // Hide client_id (Account No) for B2C locations
        if ($il->show_client_id == 1 && !$is_b2c_location) {
        ...
        // Gift order: when sent as gift with "hide prices for recipient", packing slip / receipt can hide amounts
        $output['is_gift'] = ! empty($transaction->is_gift);
        $output['hide_prices_for_recipient'] = ! empty($transaction->is_gift) && ! empty($transaction->hide_prices_for_recipient);
        ...
        //reward points - format: $X (Y points), only dollar amount is used in total
        if ($business_details->enable_rp == 1 && ! empty($transaction->rp_redeemed)) {
            ...
            $output['reward_point_points'] = (int) $transaction->rp_redeemed;
            $output['reward_point_display'] = $this->num_f($transaction->rp_redeemed_amount, $show_currency, $business_details) . ' (' . (int) $transaction->rp_redeemed . ' points)';
        }
        ...
        // Add transaction_id, invoice_token, is_quotation and location B2B info for QR code
        $output['transaction_id'] = $transaction_id;
        $output['is_quotation'] = $transaction->is_quotation ?? 0;
        // Ensure invoice_token exists for QR code URL generation
        if (empty($transaction->invoice_token)) {
            $transaction->invoice_token = $this->generateToken();
            $transaction->save();
        }
        $output['invoice_token'] = $transaction->invoice_token;
        $output['location_is_b2c'] = isset($location_details->is_b2c) ? $location_details->is_b2c : 0;
    ...
    private function getInvoiceScheme($business_id, $location_id)
    {
        ...
        if (! empty($scheme_id) && $scheme_id != 0) {
            ...
            $scheme = InvoiceScheme::where('id', $scheme_id)
                ->lockForUpdate()
                ->first();
        }
    ...
    {
        if (request() && request()->input('type') === 'sales_order') {
            return true;
        }
        ...
        foreach ($transaction_lines as $line) {
            ...
            // Gift cards are virtual: no purchase lines to map; skip to avoid purchase_sell_mismatch
            if (!empty($product->is_gift_card)) {
                continue;
            }
    ...
    public function getOpeningClosingStock($business_id, $date, $location_id, $is_opening = false, $by_sale_price = false, $filters = [], $permitted_locations = null)
    {
        ...
        $query->where('purchase.status', '!=', 'void');
    ...
    public function isCustomerTransactionLimitExeeded($input)
    {
        $final_total = $this->num_uf($input['final_total']);
        $transaction_limit = Contact::find($input['contact_id'])->transaction_limit;
       if($transaction_limit == null || $transaction_limit == 0){
        return false;
       }
       if($final_total > $transaction_limit){
        return true;
       }
        return false;
    }
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        if (! empty($transaction)) {
            ...
            if ($transaction->status == 'draft') {
                ...
                foreach ($deleted_sell_lines as $line) {
                    ...
                    $details = VariationLocationDetails::where('variation_id', $line->variation_id)->where('product_id', $line->product_id)->where('location_id', $transaction->location_id)->first();
                    if (!$details) {
                        continue;
                    }
                    
                    $temp = (int) $line->quantity;
                    // if so has completed then no effect qty 
                    if ($transaction->type != 'sales_order' && $transaction->status != 'completed') {
                        $details->increment('in_stock_qty', $temp);
                        if ($line->picked_quantity > 0 && $transaction->type == 'sales_order' && ($transaction->picking_status == 'PICKED' || $transaction->picking_status == 'PICKING')) {
                            $details->increment('qty_available', $line->picked_quantity);
                            $details->increment('in_stock_qty', $line->picked_quantity);
                        }
                    }
                }
     ...
     * @return object
     */
    public function voidSell($business_id, $transaction_id, $reason = '')
    {
        ...
        if ($transaction->status == 'void') {
            $output = [
                'success' => false,
                'msg' => 'Transaction is already voided',
            ];
            return $output;
        }
        ...
        try {
            ...
            //     $details->increment('in_stock_qty', $temp);
            //     if($transaction->type == 'sell'){
            //         $details->increment('qty_available', $temp);
            //     }
            // }
            ...
            foreach ($deleted_sell_lines as $line) {
                ...
                if ($transaction->type == 'sell') {
                    // case of overselling 
                    if ($details->qty_available < 0) {
                        Log::info('Vertual stock', ['before delete Invoice' => $transaction->invoice_no, 'before delete' => $details->in_stock_qty]);
                        Log::info('Hand stock', ['before delete Invoice' => $transaction->invoice_no, 'before delete' => $details->qty_available]);
                        $remaining = TransactionSellLine::with(['transaction'])
                        ...
                        $details->in_stock_qty = max(0, $details->qty_available + $line->quantity - $remaining);
                        $details->save();
                    } else {
                        $details->in_stock_qty += $line->quantity;
                    }
                } else {
                    $details->increment('in_stock_qty', $line->quantity);
                }
                if ($transaction->type == 'sell') {
                    $details->increment('qty_available', $line->quantity);
                }
                $details->save();
                Log::info('Vertual stock', ['after delete Invoice' => $transaction->invoice_no, 'after delete' => $details->in_stock_qty]);
                Log::info('Hand stock', ['after delete Invoice' => $transaction->invoice_no, 'after delete' => $details->qty_available]);
            }
            $this->updateCustomerRewardPoints($transaction->contact_id, 0, $transaction->rp_earned, 0, $transaction->rp_redeemed);
            ...
            $paymentData = [
                ...
                'gateway' => '',
                'note' => "{$transaction->invoice_no} was voided and {$amount_paid} was refunded to wallet",
    ...
    public function getTotalExpenses($contact_id, $status = 'both')
    {
        ...
        if ($status == 'due') {
            ...
            $query->whereIn('payment_status', ['partial', 'paid'])
                ->whereNotNull('payment_status');
        }

        $expenses = $query->sum('final_total');

        if ($status != 'due') {
            // Include paid amounts for 'paid' and 'both' statuses
            $paid = Transaction::where('contact_id', $contact_id)
                ->where('type', 'expense')
                ->whereHas('payment_lines')
     ...
     * @return object
     */
    public function getListSells($business_id, $sale_type = 'sell', $picking = false)
    {
        if ($picking) {
            ...
            $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->select(
                    ...
                    'transactions.picking_status',
                    'transactions.pickerID',
                    'transactions.priority',
                    'transactions.isVerified',
                    'transactions.isPicked',
                    'transactions.verifierID',
                    'transactions.picking_started_at',
                    'transactions.picking_ended_at',
                    'u.id as uid',
                    'u.username as uname',
            ...
            if ($sale_type == 'sell') {
                $sells->whereIn('transactions.status', ['final', 'void']);
            }
        ...
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            ->select(
                ...
                DB::raw('SUM(tsl.quantity - tsl.so_quantity_invoiced) as so_qty_remaining'),
                'transactions.is_export',
                'transactions.shipping_station_id',
                DB::raw("CONCAT(COALESCE(dp.surname, ''),' ',COALESCE(dp.first_name, ''),' ',COALESCE(dp.last_name,'')) as delivery_person")
            );
    ...
    {
        // Accept either a Contact model or an ID; fail soft if contact is missing
    ...
    public function payContact($request, $format_data = true, $is_ecom = false)
    {
        ...
        if ($format_data) {
            // Fix Carbon parsing error by ensuring date is in correct format
            try {
                $inputs['paid_on'] = \Carbon\Carbon::parse($inputs['paid_on'])->format('Y-m-d H:i:s');
            } catch (\Exception $e) {
                $inputs['paid_on'] = now()->format('Y-m-d H:i:s');
            }
        ...
        if (! $is_reverse) {
            // Only mark as advance when the explicit "advance" method is used.
            // Normal cash/card/etc payments should reduce invoice dues first.
            $inputs['is_advance'] = ($inputs['method'] === 'advance') ? 1 : 0;
        }
        ...
        // Resolve contact from either primary key or external contact_id or from related transactions.
        // If we still can't resolve it, we continue using the raw contact_id so that
        // payAtOnce() can still try to apply against transactions; we just skip wallet logic.
        $contact = null;
        if (! empty($contact_id)) {
            // Try by primary key first
            $contact = Contact::find($contact_id);
            // Fallback: some flows may pass the external contact_id string (e.g. CO00007)
            if (! $contact) {
                $contact = Contact::where('contact_id', $contact_id)->first();
            }
        }
        // If still not found, try to infer from the first transaction being paid
        if (! $contact) {
            $txIds = [];
            $transactionsInput = $request->input('transactions');
            if (is_array($transactionsInput) && ! empty($transactionsInput)) {
                $txIds = array_keys($transactionsInput);
            }
            if (! empty($txIds)) {
                $firstTx = Transaction::find(reset($txIds));
                if ($firstTx) {
                    $contact = Contact::find($firstTx->contact_id);
                    if ($contact) {
                        $contact_id = $firstTx->contact_id;
                        $inputs['payment_for'] = $contact_id;
                    }
                }
            }
        ...
        // Wallet / type logic depends on whether we could resolve the contact model
        if ($contact) {
            $advanceAmount = $contact->balance;
            ...
            // Fallback: no contact model, assume normal customer sell flow with no existing advance
            $advanceAmount = 0;
            $due_payment_type = $request->input('due_payment_type') ?: 'sell';
            $prefix_type = $due_payment_type == 'purchase' ? 'purchase_payment' : 'sell_payment';
        }
        $ref_count = $this->setAndGetReferenceCount($prefix_type, $business_id);
        //Generate reference number
        $payment_ref_no = $this->generateReferenceNumber($prefix_type, $ref_count, $business_id);
        ...
        // $inputs['is_return'] = $due_payment_type == 'sell_return' ? 1 : 0;
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true ,$isEcom=false)
    {
        $discount = [
        ...
        if (empty($sell_return)) {
            ...
            $sell_return_data['type'] = $isEcom?'sell_return_ecom':'sell_return';
        ...
        //Todo: fix the due and paid amount for the parent transaction 
        // $parent_transaction = Transaction::find($sell->id);
        // $parent_transaction->final_total = $parent_transaction->final_total - $sell_return->final_total;
    ...
    /**
     * Get only the overall balance due for a contact
     * 
     * @param int $contact_id
     * @param int $location_id
     * @return float
     */
    public function getContactBalanceDue($contact_id, $location_id = null)
    {
        $business_id = request()->session()->get('user.business_id');

        //Get Overall transaction sums
        $overall_transaction_sums = $this->__transactionQuery($contact_id, null, null, $location_id)
            ->select(
                DB::raw("SUM(IF(type = 'purchase', final_total, 0)) as total_purchase"),
                DB::raw("SUM(IF(type = 'sell' AND status = 'final', final_total, 0)) as total_invoice"),
        ...
        $total_overall_invoice = $overall_transaction_sums->total_invoice - $overall_transaction_sums->total_sell_return +
            $overall_transaction_sums->total_opening_balance - $overall_transaction_sums->total_ledger_discount;
        $total_overall_purchase = $overall_transaction_sums->total_purchase - $overall_transaction_sums->total_purchase_return;
        $overall_ledger_discount = $overall_transaction_sums->total_ledger_discount;

        //Get Overall transaction payments
        $overall_payments = $this->__paymentQuery($contact_id, null, null, $location_id)
            ->select('transaction_payments.*', 'bl.name as location_name', 't.type as transaction_type', 'is_advance')
            ->get();
        ...
        $overall_total_ob_paid = $overall_payments->where('transaction_type', 'opening_balance')->where('is_return', 0)->sum('amount');
        $overall_total_sell_change_return = $overall_payments->where('transaction_type', 'sell')->where('is_return', 1)->sum('amount');
        // $overall_total_sell_change_return = $overall_payments->where('transaction_type', 'sell_return')->sum('amount');
        $overall_total_sell_change_return = !empty($overall_total_sell_change_return) ? $overall_total_sell_change_return : 0;
        $overall_total_invoice_paid -= $overall_total_sell_change_return;
        $overall_total_purchase_paid = $overall_payments->where('transaction_type', 'purchase')->where('is_return', 0)->sum('amount');
        $overall_total_sell_return_paid = $overall_payments->where('transaction_type', 'sell_return')->sum('amount');
        ...
        $total_overall_paid_customer = $overall_total_invoice_paid - $overall_total_sell_return_paid + $overall_total_ob_paid;
        $total_overall_paid_supplier = $overall_total_purchase_paid - $overall_total_purchase_return_paid;

        $overall_due = $total_overall_invoice + $total_overall_purchase - $total_overall_paid_customer - $total_overall_paid_supplier - $overall_ledger_discount;

        // For customers, treat available credit (contact.balance / Open Balance)
        // as reducing the net balance due shown in the header.
        $contact = \App\Contact::find($contact_id);
        if ($contact && in_array($contact->type, ['customer', 'both'])) {
            $overall_due -= (float) $contact->balance;
        }
