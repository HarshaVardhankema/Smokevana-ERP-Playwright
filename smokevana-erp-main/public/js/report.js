$(document).ready(function() {
    ...
    //contact report
    var is_customer_to_supplier = "ALL";
    supplier_report_tbl = $('#supplier_report_tbl').DataTable({
        ...
        columnDefs: [
            ...
            { targets: [1, 2, 3, 4, 5, 6], orderDataType: 'dom-data-attr', type: 'num' },
        ],
        columns: [
            { data: 'name', name: 'name' },
            ...
            { data: 'opening_balance_due', name: 'opening_balance_due_value' },
            { data: 'due', name: 'due_value' },
        ],
        buttons: [
                    {
                    ...
                    {
                        text: is_customer_to_supplier,
                        className: 'tw-dw-btn-xs tw-dw-btn tw-dw-btn-outline tw-bg-gray-500 tw-text-red-500 tw-my-2 customer_to_supplier_btn',
                    }
                ],

        fnDrawCallback: function(oSettings) {
            var total_purchase = sum_table_col($('#supplier_report_tbl'), 'total_purchase');
            $('.footer_total_purchase').text(__currency_trans_from_en(total_purchase));

            var total_purchase_return = sum_table_col(
                $('#supplier_report_tbl'),
                'total_purchase_return'
            );
            $('.footer_total_purchase_return').text(__currency_trans_from_en(total_purchase_return));

            var total_sell = sum_table_col($('#supplier_report_tbl'), 'total_invoice');
            $('.footer_total_sell').text(__currency_trans_from_en(total_sell));

            var total_sell_return = sum_table_col($('#supplier_report_tbl'), 'total_sell_return');
            $('.footer_total_sell_return').text(__currency_trans_from_en(total_sell_return));

            var total_opening_bal_due = sum_table_col(
                $('#supplier_report_tbl'),
                'opening_balance_due'
            );
            $('.footer_total_opening_bal_due').text(__currency_trans_from_en(total_opening_bal_due));

            var total_due = sum_table_col($('#supplier_report_tbl'), 'total_due');
            $('.footer_total_due').text(__currency_trans_from_en(total_due));
    ...
    if($('#supplier_report_tbl').length != 0){
        $('#cnt_customer_group_id, #contact_type, #cs_report_location_id, #scr_contact_id').change(function() {
            is_customer_to_supplier = $('#contact_type').val();
            $('.customer_to_supplier_btn').text(is_customer_to_supplier);
            supplier_report_tbl.ajax.reload();
        });
    }
    $('.customer_to_supplier_btn').click(function() {
        if(is_customer_to_supplier == "ALL"){
            $('#contact_type').val('customer').trigger('change');
        }else if(is_customer_to_supplier == "customer"){
            $('#contact_type').val('supplier').trigger('change');
        }else if(is_customer_to_supplier == "supplier"){
            $('#contact_type').val('customer').trigger('change');
        }else{
            $('#contact_type').val('ALL').trigger('change');
        }
    });
    ...
    var stock_report_cols = [
            ...
            { data: 'stock', name: 'stock_hand', searchable: false },
            { data: 'webstock', name: 'stock', searchable: false },
        ];
    ...
    product_purchase_report = $('table#product_purchase_report_table').DataTable({
        ...
        fnDrawCallback: function(oSettings) {
            $('.footer_subtotal').text(__currency_trans_from_en(sum_table_col($('#product_purchase_report_table'), 'row_subtotal')));
    ...
    sell_payment_report = $('table#sell_payment_report_table').DataTable({
        ...
        fnDrawCallback: function(oSettings) {
            var total_amount = __currency_trans_from_en(sum_table_col($('#sell_payment_report_table'), 'paid-amount'));
            $('.footer_total_amount').text(total_amount);
