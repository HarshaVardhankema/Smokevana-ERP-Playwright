$(document).ready(function() {
    var productsListUrl = (typeof base_path !== 'undefined' ? base_path : '') + '/products/list';
    ...
    if ($('#search_product_for_srock_adjustment').length > 0) {
        ...
        $searchInput.autocomplete({
            source: function(request, response) {
                ...
                if (!location_id) {
                    if (typeof toastr !== 'undefined' && (typeof LANG !== 'undefined' && LANG.select_location)) {
                ...
                $.getJSON(productsListUrl, { location_id: location_id, term: request.term }, response)
                    .fail(function() {
                        response([]);
                    });
            },
            ...
            response: function(event, ui) {
                ...
                } else if (ui.content.length == 0 && $('#location_id').val()) {
    ...
    $('select#location_id').on('change select2:select', function() {
        $('table#stock_adjustment_product_table tbody').find('tr.product_row').remove();
        if ($('#search_product_for_srock_adjustment').length) {
