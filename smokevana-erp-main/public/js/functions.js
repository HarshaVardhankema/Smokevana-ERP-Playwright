...
function __write_number(
    ...
    // Check if the element is for quantity
    if (input_element.hasClass('input_quantity')) {
        precision = 0; // Set precision to 0 for integer values
    }

    // For quantity inputs, format without thousand separators (no commas)
    if (input_element.hasClass('input_quantity')) {
        // Temporarily override thousand separator for quantity formatting
        var original_thousand = __currency_thousand_separator;
        __currency_thousand_separator = '';
        input_element.val(__number_f(value, false, use_page_currency, precision));
        __currency_thousand_separator = original_thousand;
    } else {
        // Format and write the value normally
        input_element.val(__number_f(value, false, use_page_currency, precision));
    }
}
...
function __page_leave_confirmation(form) {
    ...
    // Intercept browser back/forward buttons and other navigation
    window.onbeforeunload = function(e) {
        ...
        if (form_obj.serialize() != orig_form_data) {
            ...
            // The actual SweetAlert will be shown via link click interception above
            return LANG.sure || "Changes you made may not be saved.";
        }
    }
}
