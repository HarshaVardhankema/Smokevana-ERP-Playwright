...
$(document).ready(function () {
    ...
    // console.log('dataaaa',__default_datatable_page_entries)
    //Datables
    jQuery.extend($.fn.dataTable.defaults, {

        //Uncomment below line to enable save state of datatable.'<"row margin-bottom-20 text-center"<"col-sm-1"l><"col-sm-8"B><"col-sm-3"f> r>tip',
        //stateSave: true,
        fixedHeader: true,
        dom: '<"tw-flex tw-flex-wrap tw-gap-6 tw-mb-0"<""l><""f><""B> r>tip'
        ,
        buttons: buttons,
        aLengthMenu: [
...
$(document).on('keydown', 'input[name="quantity"]', function (e) {
    const key = e.key;
    if (key === '.' || key === ',' || (e.shiftKey || (key < '0' || key > '9'))) {
        if (
...
$(document).on('click', '.sku-barcode-copy-icon', function(){
