...
<tr class="product_row" ml="{{ $product->ml }}" ct="{{ $product->ct }}"
    ...
    <td style="width: 80px; margin:auto; text-align: center;">
        <img src="@if (count($product->media) > 0) {{ $product->media->first()->display_url }}
            ...
            style="height: 100%; display: block; margin: 5px auto; border: black; border-radius: 5px; width: 50px; object-fit: cover;">
    </td>
    ...
    <td style="width: 600px;">
        <div class="row">
            ...
            <div class="col-sm-2"><small class="text-muted p-1">
                    @if ($product->enable_stock)
                        {{ number_format($product->qty_available, 0) }} {{ $product->unit }} @lang('lang_v1.in_stock')
                    @else
                        --
    ...
    <td>
        ...
        <div style="display: flex; justify-content: center; align-items: center;">
            <div class="input-group input-number" style="width: 120px;">
                ...
                <input type="text" data-min="1" oninput="this.value = this.value.replace(/[^0-9]/g, '')"
                    class="form-control pos_quantity input_number mousetrap input_quantity text-center"
                    value="{{ $product->quantity_ordered }}" name="products[{{ $row_count }}][quantity]"
    ...
    @if (!empty($is_direct_sell))
        ...
        @endphp
        <td style="width: 120px;" class="hide text-center @if (!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
            <input type="text" name="products[{{ $row_count }}][unit_price]"
                class="form-control pos_price_per_unit input_number mousetrap text-center"
        ...
        <td style="width: 120px;" class="text-center @if (!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
            <input type="text" name="products[{{ $row_count }}][unit_price]"
                class="form-control pos_unit_price input_number mousetrap text-center"
                value="{{ @num_format($pos_unit_price) }}"
                @if (!empty($pos_settings['enable_msp'])) data-rule-min-value="{{ $pos_unit_price }}" data-msg-min-value="{{ __('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($pos_unit_price)]) }}" @endif>
            <br>
            <p class="hide">Initial Price: <span
                    class="old_pos_unit_price">{{ @num_format($pos_unit_price) }}</span></p>
        ...
        <td class="hide" @if (!$edit_discount)  @endif>
            ...
            {!! Form::select(
                ...
                $discount_type,
                ['class' => 'form-control row_discount_type', 'style' => 'min-width: 90px; width: 100%;'],
            ) !!}
            @if (!empty($discount))
    ...
    <td class="{{ $hide_tax }}">
        <input type="text" name="products[{{ $row_count }}][unit_price_inc_tax]"
            ...
            value="{{ @num_format($unit_price_inc_tax) + @num_format($item_tax) }}"
            @if (!$edit_price) readonly @endif
            @if (!empty($pos_settings['enable_msp'])) data-rule-min-value="{{ $unit_price_inc_tax + $item_tax }}" data-msg-min-value="{{ __('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($unit_price_inc_tax) + @num_format($item_tax)]) }}" @endif>
    </td>
    ...
    <td class="text-center" id="erpTax">
        ...
        <span class="display_currency pos_taxation_total_text @if (!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif"
            data-currency_symbol="true">{{ $item_tax }}</span>
    </td>
    <td class="text-center">
        ...
        <span class="display_currency pos_line_total_text @if (!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif"
            data-currency_symbol="true">{{ $product->quantity_ordered * ($unit_price_inc_tax + $item_tax) }}</span>
    </td>
