    ...
    <div class="tab-content">
        @foreach($templates as $key => $value)
            <div class="tab-pane @if($loop->index == 0) active @endif" id="cn_{{$key}}">
                ...
                @if($key == 'new_sale' || $key == 'payment_reminder'|| $key == 'shipment'|| $key == 'local_pickup'||$key == 'forget_password' ||$key == 'password_reset_success' ||$key == 'subscribe_newsletter' ||$key == 'registration_confirmation' ||$key == 'contact_us_success' ||$key == 'test_notification'||$key=='email_confirmation' ||$key=='credit_approved'||$key=='order_packed'||$key=='shipped'||$key=='stock_alert'||$key=='order_shipped'||$key=='gift_card_code'||$key == 'forget_password_web')
