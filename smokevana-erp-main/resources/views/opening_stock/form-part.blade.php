    ...
    <div class="col-sm-12">
        @forelse($locations as $key => $value)
            <div class="box box-solid">
                ...
                <div class="box-body">
                    <div class="row">
                        <div class="table-responsive tw-overflow-y-auto" style="max-height: 50vh;">
                            <table
                                class="table table-condensed table-bordered text-center table-responsive table-striped add_opening_stock_table">
                                <thead class="text-nowrap">
                                    <tr>
                                        <th style="width: 30%;">@lang('product.product_name')</th>
                                        <th style="width: 7%;">In Hand Remaining</th>
                                        <th style="width: 7%;">Stock Available</th>
                                        ...
                                        @if ($enable_lot == 1)
                                            <th style="width: 6%;">@lang('lang_v1.lot_number')</th>
                                        @endif
                        
                                        <th style="width: 8%;">@lang('purchase.subtotal_before_tax')</th>
                                        <th style="width: 6%;">@lang('lang_v1.date')</th>
                                        <th style="width: 10%;">@lang('brand.note')</th>
                                        <th style="width: 2%;">&nbsp;</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    @foreach ($product->variations as $variation)
                                        ...
                                        @foreach ($purchases[$key][$variation->id] as $sub_key => $var)
                                            ...
                                            <tr>
												...
												<!-- Add custom start column for in_stock_qty -->
												<td>
													@php
														$in_stock_qty = 0;
														$in_stock_qty_id = null;
														foreach ($variation->variation_location_details as $location_detail) {
															if ($location_detail->location_id == $key) {
																$in_stock_qty = $location_detail->in_stock_qty;
																$in_stock_qty_id= $location_detail->id;
																break;
															}
														}
													@endphp
													{!! Form::text(
														'stocks[' . $key . '][' . $variation->id . '][' . $sub_key . '][in_stock_qty]',
														@format_quantity($in_stock_qty),
														['class' => 'form-control input-sm input_number qty_quantity input_quantity', 'required'],
													) !!}
												</td>
                                                <td>
                                                    {!! Form::text(
                                                ...
                                                <td>
                                                    @if ($loop->index == 0)
                                                        <button type="button"
                                                            class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-primary add_stock_row"
                                                            data-sub-key="{{ count($purchases[$key][$variation->id]) }}"
                                                            data-row-html='<tr><td>{{ $product->name }} @if ($product->type == 'variable')
