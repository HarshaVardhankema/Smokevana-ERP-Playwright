...
<tr class="product_row" recallPrice="{{ $product?->recall_price ?? $recall_price ?? null }}" ml="{{ $product->ml }}" ct="{{ $product->ct }}" cost="{{ $product?->default_purchase_price ?? 0 }}"
    pricegroup="{{ json_encode($product->group_prices) }}" locationTaxType="{{ $product->locationTaxType ?? '[]' }}"
    data-row_index="{{ $row_count }}"
    @if (!empty($so_line)) data-so_id="{{ $so_line->transaction_id }}" @endif>
    {{-- Hidden placeholder for old # column so header alignment stays correct --}}
    <td class="so-hide"></td>

    {{-- Product column: image + details in a single cell --}}
    <td style="width: 40%;">
        <div style="display: flex; align-items: flex-start; gap: 8px;">
            <div>
                <img src="@if (count($product->media) > 0) {{ $product->media->first()->display_url }}
                    ...
                    style="display: block; border-radius: 5px; width: 30px; height: 30px; object-fit: cover;">
            </div>

            <div class="product_name_class" style="flex: 1;">
                @if (!empty($so_line))
...
if (!empty($product->brand)) {
    $product_name .= ' ' . $product->brand;
                ...
                @else
                   @if ($product->var_barcode_no)
                   <strong>{{$product->var_barcode_no}} </strong>   
                   @endif
                   <strong>{{$product->sub_sku}}</strong>
                ...
                <input type="hidden" class="sub_sku" name="products[{{ $row_count }}][sub_sku]" value="{{ $product->sub_sku }}">
...
if ($discount_type == 'fixed') {
    $discount_amount = $discount_amount * $multiplier;
                ...
                {{-- this is for qty_available --}}


                {{-- <small class="text-muted p-1">
			@if ($product->enable_stock)
			{{ number_format($product->qty_available, 0) }} {{$product->unit}} @lang('lang_v1.in_stock')
			@else
			--
			@endif
		</small> --}}
            ...
            'product_ms' => $product->product_ms,
                ...
                @php
// Only set max_quantity based on stock if enable_stock is true
if ($product->enable_stock) {
    ...
    // If stock is disabled, set unlimited quantity
    $max_quantity = null;
    $formatted_max_quantity = '--';
}
...
$max_qty_msg = __('validation.custom-messages.quantity_not_available', [
    ...
    if (!empty($product->lot_no_line_id)) {
        $lot_no_line_id = $product->lot_no_line_id;
            ...
            if ($lot_number->purchase_line_id == $lot_no_line_id) {
                ...
                // Only set lot quantity rule if enable_stock is true
                if ($product->enable_stock) {
            ...
            if ($exp_enabled == 1 && !empty($lot_number->exp_date)) {
                if (
                    \Carbon::now()->gt(
                        \Carbon::createFromFormat('Y-m-d', $lot_number->exp_date),
                    )
                ) {
            ...
            if (
                !empty($purchase_line_id) &&
                $purchase_line_id == $lot_number->purchase_line_id
            ) {
                $selected = 'selected';
            ...
            {{-- <div><small class="text-muted p-1">
                    @if ($product->enable_stock)
                        {{ number_format($product->qty_available, 0) }} {{ $product->unit }} @lang('lang_v1.in_stock')
                    @else
                        --
    ...
    <td class="text-center">
         @if ($product->enable_stock)
            {{ number_format($product->qty_available, 0) }}
         @else
            --
         @endif
    </td>
...
if ($product->unit_allow_decimal != 1) {
    $allow_decimal = false;
        ...
        @foreach ($sub_units as $key => $value)
            @if (!empty($product->sub_unit_id) && $product->sub_unit_id == $key)
                @php
        // Only adjust max_qty_rule if enable_stock is true
        if ($product->enable_stock && $max_qty_rule !== null) {
        ...
        <div style="justify-content: center; align-items: center;">
            <div class="input-group input-number" style="width: 150px;">
                 @php
// Only calculate max_qty_value based on stock if enable_stock is true
if ($product->enable_stock) {
    if ($isOverSelling) {
        if ($overSellingQuantity) {
            $max_qty_value = $product->qty_available + $overSellingQuantity;
        } else {
            $max_qty_value = $product->qty_available + 5000;
        }
    } else {
        $max_qty_value = $product->qty_available;
    }
} else {
    // If stock is disabled, set a very high value to allow unlimited quantity
    $max_qty_value = 999999999;
}
                ...
                <input type="text" @if (!empty($is_delete_not_allowed)) data-min="0" @else data-min="1" @endif 
                    class="form-control pos_quantity input_number mousetrap input_quantity text-center"
                    oninput="this.value = this.value.replace(/[^0-9]/g, '')"
                    value="{{number_format($product->quantity_ordered, 0, '.', '') }}" name="products[{{ $row_count }}][quantity]"
                    data-allow-overselling="@if (empty($pos_settings['allow_overselling'])) {{ 'false' }}@else{{ 'true' }} @endif"
                    @if ($allow_decimal) data-decimal=1
					@else
                    @if ($product->enable_stock) data-maxvalue={{$max_qty_value}} @endif
					data-decimal=0
                    @if (!empty($is_delete_not_allowed)) min="0" @else min="1" @endif
                    @if ($product->enable_stock) max={{$max_qty_value}} @endif
                    ...
                    @if ($product->enable_stock)
                        @if (empty($pos_settings['allow_overselling']) && empty($is_sales_order))
                            data-rule-max-value="{{$max_qty_value}}" data-msg-max-value="{{ $max_qty_msg }}"
                        @endif
    ...
    {{-- Unit Price column (always shown) --}}
    @php
    ...
    <td class="text-center">
        <div class="input-group" style="width:150px; margin: 0 auto;">
            ...
            <input type="text" name="products[{{ $row_count }}][unit_price]"
                ...
                min="0.01"
                @if (!auth()->user()->can('edit_product_price_from_sale_screen')) disabled @endif
        ...
        <p class="hide">Initial Price: <span class="old_pos_unit_price">{{ @num_format($pos_unit_price) }}</span></p>
        <p class="hide">Tax Per Unit : <span class="old_pos_unit_price_tax"> </span> </p>
    </td>

    {{-- Discount column (always shown) --}}
    <td class="text-center">
        @php
            $is_disabled = !auth()->user()->can('edit_product_discount_from_sale_screen') ? true : false;
        @endphp
        <div class="discount-input-wrap">
            <input type="text"
                name="products[{{ $row_count }}][line_discount_amount]"
                value="{{ @num_format($discount_amount) }}"
                class="form-control input_number row_discount_amount discount-amt"
                max="100" min="0"
                @if($is_disabled) disabled @endif>
            <select name="products[{{ $row_count }}][line_discount_type]"
                class="row_discount_type discount-type-sel"
                @if($is_disabled) disabled @endif>
                <option value="fixed" {{ $discount_type == 'fixed' ? 'selected' : '' }}>$</option>
                <option value="percentage" {{ $discount_type != 'fixed' ? 'selected' : '' }}>%</option>
            </select>
        </div>
        @if (!empty($discount))
    ...
    {{-- Tax column (conditional, hidden) --}}
    <td class="so-hide text-center {{ $hide_tax }}">
...
<td class="so-hide {{ $hide_tax }}">
    @php
$total_unit_price_inc_tax = (float) $unit_price_inc_tax + (float) $item_tax;
@endphp
    <input type="text" name="products[{{ $row_count }}][unit_price_inc_tax]"
        class="form-control pos_unit_price_inc_tax input_number"
        value="{{ @num_format($total_unit_price_inc_tax) }}"
        @if (!$edit_price) readonly @endif
        @if (!empty($pos_settings['enable_msp']))
            data-rule-min-value="{{ $total_unit_price_inc_tax }}"
            data-msg-min-value="{{ __('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($total_unit_price_inc_tax)]) }}"
        @endif>
</td>
    ...
    {{-- Tax Per Unit column --}}
    <td class="text-center" id="erpTax">
        @php
            $subtotal_type = !empty($pos_settings['is_pos_subtotal_editable']) ? 'text' : 'hidden';
        @endphp
        <input type="{{ $subtotal_type }}"
            class="form-control pos_taxation_total @if (!empty($pos_settings['is_pos_subtotal_editable'])) input_number @endif"
            value="{{ @num_format($item_tax) }}">
        <span class="pos_taxation_total_text @if (!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif"
            style="display: inline-block; font-weight: 500; color: #0F1111;">
            ${{ number_format((float)$item_tax, 2, '.', '') }}
        </span>
    </td>
    ...
    <td class="text-center">
        ...
        <input type="{{ $subtotal_type }}"
            ...
            value="{{ @num_format($product->quantity_ordered * ($unit_price_inc_tax + $item_tax)) }}">
        <span class="pos_line_total_text @if (!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif"
            style="display: inline-block; font-weight: 500; color: #0F1111;">
            ${{ number_format((float)($product->quantity_ordered * ($unit_price_inc_tax + $item_tax)), 2, '.', '') }}
        </span>
    </td>

    <td class="hide text-center v-center">
        @if (!empty($is_direct_sell))
            <br>
    ...
    <td class="text-center">
        @if (!empty($is_delete_not_allowed)&& !$is_sales_order)
        <i class="fa fa-times text-danger pos_remove_row_WA cursor-pointer" aria-hidden="true"></i>
        @else
