    ...
    $(document).ready(function () {
                                            ...
                                            $.ajax({
                                                ...
                                                success: function (
                                                    ...
                                                    $('.view_modal')
                                                        ...
                                                        $('#modal_shipment_packing_modal').modal('hide');
                                                },
                                                error: function () {
                                                    toastr
                                                        .error(
        ...
        $("#cost_review").on("click", function () {
            ...
            let data = {
                ...
                "to_country_code": user_country,
                "to_postal_code": user_zipCode,
                "to_city_locality": user_city,
                "to_state_province": user_state,
                "weight": {
                    "value": weightInGrams,
                    "unit": 'gram'
                },
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.status) {
                            ...
                            // Handle nested error structure from shipstation
                            if (response.message && typeof response.message === 'object') {
                                // Format the complete error object with proper indentation
                                ...
                                // If no specific error message, show the entire response
                                errorMessage = JSON.stringify(response, null, 2);
                            }
                            
                            // Debug: log the response to console
                            console.log('Shipstation Error Response:', response);
                            console.log('Formatted Error Message:', errorMessage);
                            
                            // Create a DOM element for the error content
                            let errorDiv = document.createElement('div');
                            errorDiv.style.cssText = 'text-align: left; max-height: 400px; overflow-y: auto; overflow-x: auto; width: 100%; margin-top: 20px;';
                            
                            let errorPre = document.createElement('pre');
                            errorPre.style.cssText = 'white-space: pre-wrap; word-wrap: break-word; background: #f5f5f5; padding: 15px; border-radius: 5px; border: 1px solid #ddd; margin: 0; font-family: monospace; font-size: 12px; line-height: 1.5; text-align: left; max-width: 100%;';
                            errorPre.textContent = errorMessage;
                            
                            errorDiv.appendChild(errorPre);
                            
                            swal({
                                title: "Your shipstation is Saying",
                                content: errorDiv,
                                icon: "error",
                                buttons: {
                                    confirm: {
