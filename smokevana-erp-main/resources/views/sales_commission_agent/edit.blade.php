<div class="modal-dialog modal-xl" role="document" data-backdrop="static" data-keyboard="false">
  <div class="modal-content sca-amz">
    ...
    <style>
      /* Keep edit consistent with the new create look */
      ...
      .sca-amz .modal-title { font-weight: 800; letter-spacing: 0.2px; }
      .sca-amz .modal-body { background: #f6f7fb; padding-top: 18px; }
      .sca-amz .amz-subtitle { color: rgba(255,255,255,.75); font-size: 12px; margin-top: 4px; }
      .sca-amz .amz-actions .tw-dw-btn { border-radius: 999px; padding: 8px 14px; }
      .sca-amz .amz-card {
        background: #fff; border: 1px solid #e5e7eb; border-radius: 14px;
        padding: 14px 14px 6px; margin-bottom: 14px; box-shadow: 0 1px 2px rgba(0,0,0,.04);
      }
      .sca-amz .amz-card h4 { font-size: 14px; margin: 0 0 12px; font-weight: 800; color: #111827; display:flex; align-items:center; gap:8px; }
      .sca-amz .amz-card h4 .amz-pill { font-size:11px; font-weight:700;; padding:2px 8px; border-radius:999px; border:1px solid #e0e7ff; }
      ...
      .sca-amz .select2-container--default .select2-selection--multiple {
        border-radius: 10px !important; border-color:#d1d5db; box-shadow:none; min-height:38px;
      }
      .sca-amz textarea.form-control { min-height: 88px; }
      .sca-amz .form-group label { font-size: 12px; font-weight: 700; color: #374151; }
      .sca-amz .help-block { margin-top: 6px; color: #6b7280; font-size: 12px; }
      .sca-amz .amz-divider { height:1px; background:#e5e7eb; margin:12px 0 16px; }
      .sca-amz .amz-inline { display:flex; align-items:center; gap:10px; flex-wrap:wrap; }
      .sca-amz .amz-inline .checkbox { margin: 0; }
      /* Hide advanced commission fields (match 2nd reference) */
      .sca-amz .sca-advanced-commission { display: none; }
    ...
    <div class="modal-header">
      <div class="tw-flex tw-items-start tw-justify-between tw-gap-4">
        <div>
          <h4 class="modal-title" id="modalTitle">@lang( 'lang_v1.edit_sales_commission_agent' )</h4>
          <div class="amz-subtitle">Update profile, access, and commission settings.</div>
        </div>
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  @php
                    $selected_customers = [];
                    if (isset($user) && $user->contactAccess) {
                        $selected_customers = $user->contactAccess->pluck('id')->toArray();
                    }
                  @endphp
                  {!! Form::select('b2b_customers[]', [], $selected_customers, ['class' => 'form-control select2', 'multiple' => 'multiple', 'id' => 'b2b_customers', 'style' => 'width: 100%']) !!}
              ...
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  {!! Form::text('max_discount_percent', @num_format($user->max_discount_percent), ['class' => 'form-control input_number', 'placeholder' => 'Max Discount %', 'min' => '0', 'max' => '100', 'step' => '0.01']); !!}
                </div>
              </div>
              <div class="col-md-3">
                <div class="form-group">
              ...
              <div class="col-md-6 col-md-offset-6 sca-advanced-commission">
                <div class="form-group">
                  ...
                  {!! Form::select('commission_type', ['invoice_value' => __( 'lang_v1.invoice_value'), 'payment_received' => __( 'lang_v1.payment_received' ), 'by_category' => __( 'lang_v1.by_category')], !empty($user->commission_type) ? $user->commission_type : null, ['class' => 'form-control', 'placeholder' => __( 'lang_v1.commission_type') ]); !!}
                </div>
              </div>
              ...
              <div class="col-md-4 sca-advanced-commission">
                <div class="form-group">
                  ...
                  {!! Form::text('percentage_value', @num_format($user->percentage_value), ['class' => 'form-control input_number', 'placeholder' => __( 'lang_v1.percentage_value' ), 'required' ]); !!}
                </div>
              </div>
              <div class="col-md-2">
                <div class="form-group">
                  ...
                  <div class="amz-inline">
                    <div class="checkbox">
                      <label style="font-weight: 600;">
                        ...
                        {!! Form::checkbox('status', 'active', !empty($user->status) ? ($user->status == 'active') : true, ['id' => 'status']); !!}
                        @lang('is Active')
                      </label>
                    </div>
                  </div>
                    ...
                    <label style="font-weight: 600;">
                      @php
                        ...
                        if (is_array($permitted_locations)) {
                            $is_checked = in_array($location->id, $permitted_locations);
                        } elseif ($permitted_locations == 'all') {
                            $is_checked = true;
                        }
                      @endphp
                      {!! Form::checkbox('location_permissions[]', 'location.' . $location->id, $is_checked, ['class' => 'input-icheck']) !!}
...
$(document).ready(function(){
    ...
    $('#b2b_customers').select2({
        ajax: {
            url: '{{ action("App\Http\Controllers\ContactController@getCustomers") }}',
            dataType: 'json',
            delay: 250,
    ...
    // Load existing selected customers
    @if(isset($user) && $user->contactAccess && $user->contactAccess->count() > 0)
        var selectedCustomers = @json($user->contactAccess->map(function($contact) {
            return [
                'id' => $contact->id,
                'text' => $contact->name . (!empty($contact->contact_id) ? ' (' . $contact->contact_id . ')' : '')
            ];
        }));
        
        $('#b2b_customers').select2('data', selectedCustomers);
    @endif
