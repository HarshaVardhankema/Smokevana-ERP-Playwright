<div class="modal-dialog" role="document">
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            @elseif(in_array($transaction->type, ['expense', 'expense_refund', 'commission_payout']))
                <div class="row invoice-info">
                    @if($transaction->type == 'commission_payout' && !empty($transaction->sale_commission_agent))
                        <div class="col-sm-4 invoice-col">
                            @lang('lang_v1.commission_agent'):
                            <address>
                                <strong>{{ $transaction->sale_commission_agent->first_name }} {{ $transaction->sale_commission_agent->last_name }}</strong>
                                @if(!empty($transaction->sale_commission_agent->email))
                                    <br>@lang('business.email'): {{$transaction->sale_commission_agent->email}}
                                @endif
                                @if(!empty($transaction->sale_commission_agent->contact_no))
                                    <br>@lang('lang_v1.contact_no'): {{$transaction->sale_commission_agent->contact_no}}
                                @endif
                            </address>
                        </div>
            ...
            @else
                <div class="row invoice-info">
                    <div class="col-sm-4 invoice-col">
                        @php
                            $is_b2c_location = !empty($transaction->location) && $transaction->location->is_b2c == 1;
                        @endphp
                            ...
                            <tr>
                              ...
                              @if((in_array($transaction->type, ['hms_booking']) && auth()->user()->can('hms.edit_booking_payment')) || (auth()->user()->can('edit_purchase_payment') && (in_array($transaction->type, ['purchase', 'purchase_return']))) || (auth()->user()->can('edit_sell_payment') && (in_array($transaction->type, ['sell', 'sell_return']))) || ((auth()->user()->can('all_expense.access') || auth()->user()->can('view_own_expense')) && $transaction->type == 'expense') || (auth()->user()->can('user.update') && $transaction->type == 'commission_payout'))
                                    @if($payment->method != 'advance')
                                        <button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-info edit_payment hide" 
                                ...
                                @if((in_array($transaction->type, ['hms_booking']) && auth()->user()->can('hms.delete_booking_payment')) || (auth()->user()->can('delete_purchase_payment') && (in_array($transaction->type, ['purchase', 'purchase_return']))) || (auth()->user()->can('delete_sell_payment') && (in_array($transaction->type, ['sell', 'sell_return']))) || ((auth()->user()->can('all_expense.access') || auth()->user()->can('view_own_expense')) && $transaction->type == 'expense') || (auth()->user()->can('user.update') && $transaction->type == 'commission_payout'))
                                    &nbsp; <button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-error delete_payment hide" 
