			...
			<div class="text-center">
      			<div class="btn-group">
					...
					<button type="submit" value="save_n_add_another" class="tw-dw-btn tw-text-white tw-dw-btn-lg bg-maroon submit_form">@lang('lang_v1.save_n_add_another')</button> --}}
          			<button type="submit" value="submit" class="tw-dw-btn tw-dw-btn-primary tw-text-white submit_form">@lang('messages.save')</button>
          		</div>
          	</div>
	    </div>
</section>
								...
								<thead>
									<tr>
										...
										@foreach($price_groups as $price_group)
											<th>
												<i class="fa fa-copy pull-left"></i>
												{{ Str::before(preg_replace('/([a-z])([A-Z])/', '$1 $2', $price_group->name), ' ') }} @show_tooltip(__('lang_v1.price_group_price_type_tooltip'))</th>
										@endforeach
									</tr>
								</thead>
								<tbody>
										...
										@if($product->type == 'variable')
											<td >
												...
												<div class="tw-flex tw-gap-3">
													<b>Varient:</b>  {{$variation->name}}
												</div>
												<div class="tw-flex tw-gap-3">
													<br><b>SKU:</b>  {{$variation->sub_sku}}
													@if ($variation->var_barcode_no)
													<br><b>Barcode:</b> {{$variation->var_barcode_no??' '}}
													@endif
												</div>
										...
										<td><span class="display_currency" data-currency_symbol="true">{{$variation->sell_price_inc_tax}}</span></td>
											@foreach($price_groups as $price_group)
												<td>
                                                    ...
                                                    <select name={{$name}} class="form-control hide">
