...
<section class="content no-print">
                ...
                <div class="modal-dialog modal-lg" role="document"> {{-- Use modal-lg or modal-xl as needed --}}
                    <div class="modal-content">
                        ...
                        <div class="modal-body" style="padding: 0px; margin-top: 10px;">
                            ...
                            'getStockReport']), 'method' => 'get', 'id' => 'product_sell_report_form' ]) !!}
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        select2', 'placeholder' => __('messages.please_select'), 'required']); !!}
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    !!}
                                    {!! Form::select('psr_customer_group_id', $customer_group, null, ['class' =>
                                    'form-control select2', 'style' => 'width:100%', 'id' => 'psr_customer_group_id']);
                                    !!}
                                </div>
                            </div>
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    select2', 'style' => 'width:100%', 'id' => 'psr_filter_category_id', 'placeholder'
                                    => __('lang_v1.all')]); !!}
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    'style' => 'width:100%', 'id' => 'psr_filter_brand_id', 'placeholder' =>
                                    __('lang_v1.all')]); !!}
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    'product_sr_date_filter', 'readonly']); !!}
                                </div>
                            </div>
                            <div class="col-md-3">
                                ...
                                @endphp
                                <div class="form-group">
                                    {!! Form::text('start_time', @format_time($startDay), ['style' =>
                                    __('lang_v1.select_a_date_range'), 'class' => 'form-control width-50 f-left', 'id'
                                    => 'product_sr_start_time']); !!}
                                    {!! Form::text('end_time', @format_time($endDay), ['class' => 'form-control width-50
                                    f-left', 'id' => 'product_sr_end_time']); !!}
                                </div>
                            </div>
                            {!! Form::close() !!}
                            {{-- @endcomponent --}}
    ...
    <!-- Product Sell Report Graph Section -->
    <div class="row no-print" id="product_sell_report_chart_container" style="margin-bottom: 20px;">
        <div class="col-md-12">
            @component('components.widget', ['class' => 'box-primary', 'title' => __('Monthly Product Sales Chart'), 'title_svg' => '<i class="fa fa-chart-bar"></i>'])
                <div class="chart-container" style="position: relative; height: 500px; width: 100%;">
                    <canvas id="product_sell_report_chart"></canvas>
                </div>
            @endcomponent
        </div>
    </div>
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="psr_detailed_tab">
                        <div class="table-responsive">
                            <table class="table   table-bordered table-striped ajax_view hide-footer" id="product_sell_report_table" style="min-width: max-content;">
                                <thead>
                                    <tr>
                                        <th>@lang('sale.product')</th>
                                        <th>@lang('product.sku')</th>
                                        <th id="psr_product_custom_field1">
                                            {{$product_custom_field1}}</th>
                                        <th id="psr_product_custom_field2">
                                            {{$product_custom_field2}}</th>
                    ...
                    <div class="tab-pane" id="psr_detailed_with_purchase_tab">
                        <div class="table-responsive">
                            ...
                            <table class="table   table-bordered table-striped ajax_view hide-footer"
                                id="product_sell_report_with_purchase_table" style="min-width: max-content;">
                                <thead>
                                    <tr>
                                        <th>@lang('sale.product')</th>
                                        <th>@lang('product.sku')</th>
                    ...
                    <div class="tab-pane" id="psr_grouped_tab">
                        <div>
                            <table class="table   table-bordered table-striped ajax_view hide-footer" id="product_sell_grouped_report_table"
...
<script type="text/javascript">
    var productSellReportChart = null;
    
    function loadProductSellReportChart() {
        var start_date = $('#product_sr_date_filter').data('daterangepicker') 
            ? $('#product_sr_date_filter').data('daterangepicker').startDate.format('YYYY-MM-DD')
            : null;
        var end_date = $('#product_sr_date_filter').data('daterangepicker')
            ? $('#product_sr_date_filter').data('daterangepicker').endDate.format('YYYY-MM-DD')
            : null;
        var location_id = $('#location_id').val();
        var customer_id = $('#customer_id').val();
        var customer_group_id = $('#psr_customer_group_id').val();
        var category_id = $('#psr_filter_category_id').val();
        var brand_id = $('#psr_filter_brand_id').val();
        ...
        if (category_id) requestData.category_id = category_id;
        if (brand_id) requestData.brand_id = brand_id;
        
        $.ajax({
            url: '/reports/product-sell-report-grouped-chart-data',
            method: 'GET',
            data: requestData,
            success: function(response) {
                if (response.success && response.labels && response.datasets) {
                    var ctx = document.getElementById('product_sell_report_chart');
                    if (!ctx) {
                    ...
                    if (productSellReportChart) {
                        productSellReportChart.destroy();
                    }
                    
                    // Create grouped bar chart
                    productSellReportChart = new Chart(ctx, {
                        type: 'bar',
                        data: {
                        ...
                        options: {
                            ...
                            plugins: {
                                title: {
                                    display: true,
                                    text: '@lang("Monthly Product Sales Chart")',
            ...
            error: function(xhr, status, error) {
                console.error('Error loading product sell report grouped chart data:', {xhr, status, error, response: xhr.responseJSON});
            }
        });
    }
    ...
    $(document).ready(function() {
        ...
        setTimeout(function() {
            loadProductSellReportChart();
        }, 1000);
        
        // Reload chart when filters change
        $('#location_id, #customer_id, #psr_customer_group_id, #psr_filter_category_id, #psr_filter_brand_id').on('change', function() {
            loadProductSellReportChart();
        });
