@foreach( $variations as $variation)
    <tr @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif @if(!empty($purchase_requisition_line)) data-purchase_requisition_id="{{$purchase_requisition_line->transaction_id}}" @endif >
        <td class="so-hide">
            ...
            <input type="hidden" class="hidden_variation_id" value="{{ $variation->id }}">
        </td>
    ...
    @endphp
        @if (empty($is_purchase_order))
            <td>
                <div>
                    ...
                    <strong>{{ $max_quantity }} /{{ $quantity_value }} </strong>
                </div>
            </td>
        @endif
        ...
        <td>
            @php
                ...
                // Default discount to 0 when adding a new product; only use value from existing PO line or import
                $discount_percent = 0;
                if (!empty($purchase_order_line)) {
                    $discount_percent = $purchase_order_line->discount_percent;
                } elseif (!empty($imported_data['discount_percent'])) {
                    $discount_percent = $imported_data['discount_percent'];
                }
            ...
            <div class="input-group">
                ...
                {!! Form::number('purchases[' . $row_count . '][pp_without_discount]',
          number_format($pp_without_discount, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator),
          [
        ...
        <td style="width: 150px">
            <div class="input-group" style="display: flex; flex-wrap: nowrap;">
                {!! Form::text('purchases[' . $row_count . '][discount_percent]', number_format($discount_percent, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'form-control input-sm inline_discounts input_number', 'required', 'min' => 0,'style'=>'width:100px; border-top-right-radius: 0; border-bottom-right-radius: 0;',
                'step' => 'any','max' =>$pp_without_discount]);  !!}
                <select name="purchases[{{$row_count}}][row_discount_type]" class="form-control input-sm inline_discounts" style="min-width: 56px; width: 56px; border-top-left-radius: 0; border-bottom-left-radius: 0;">
        ...
        <div class="input-group">
            ...
            number_format($purchase_price, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'form-control input-sm purchase_unit_cost input_number', 'required', 'min' => '0','readonly' => true,
            'step' => 'any',]); !!}
        </div>
            </td>
        @else
        {{-- purchase_price required for Add Purchase Receipt; use purchase_price_inc_tax when tax hidden --}}
        {!! Form::hidden('purchases[' . $row_count . '][purchase_price]', number_format($purchase_price, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'purchase_unit_cost purchase_price_field']); !!}
        @endif
        <td class="{{$hide_tax}} @if(!empty($is_purchase_order)) po-compact-hide @endif">
        ...
        @endif
        @if(empty($is_purchase_order) && session('business.enable_product_expiry'))
