				...
				<table class="table bg-gray" id="purchase_return_table">
			        ...
			        <tbody>
			            ...
			            <tr>
			              	...
			              	<td>
			              		<input type="text" name="return_prices[{{$purchase_line->id}}]" 
			              			value="{{@num_format($purchase_line->purchase_price_inc_tax)}}"
			              			class="form-control input-sm input_number return_price"
			              			data-rule-abs_digit="false">
			              	</td>
...
<script type="text/javascript">
	...
	$(document).on('change', 'input.return_qty, input.return_price', function(){
		update_purchase_return_total()
	});
	...
	function update_purchase_return_total(){
		...
		$('table#purchase_return_table tbody tr').each( function(){
			...
			var unit_price = __read_number($(this).find('input.return_price'));
