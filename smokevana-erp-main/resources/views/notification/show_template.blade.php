  ...
  <div class="modal-content">
    ...
    <div class="tw-flex tw-flex-wrap tw-items-center tw-gap-2">
        <h3 class="tw-text-base sm:tw-text-lg tw-text-white">
            <b>@lang( 'lang_v1.send_notification' ) - {{$template_name}}</b>
        </h3>
    </div>
    ...
    <div class="modal-body">
        ...
        @endif
        @if ($notification_template['template_for'] == 'registration_approve_confirmation')
...
// Fix for not updating textarea value on modal
  ...
  $('#send_notification_form').submit(function(e){
        e.preventDefault();
        let template_name=@json($template_name);

    tinyMCE.triggerSave();
    var data = $(this).serializeArray();
    if (template_name === 'Send Payment Notification') {
      data.push({
        name: 'paymentData',
        value: JSON.stringify(@json($transaction ?? null))
    });
  }
  data = $.param(data); 
