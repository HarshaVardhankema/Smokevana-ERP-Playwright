...
<div class="col-md-6 col-sm-6 col-xs-6 text-right align-right @if(!empty($for_pdf)) width-50 f-left @endif">
	...
	<div style="border: 1px solid #000; padding: 10px;">
		...
		<table class="table table-condensed text-left align-left no-border @if(!empty($for_pdf)) table-pdf @endif">
			...
			<tr >
				<td><strong>@lang('lang_v1.balance_due')</strong></td>
				<td class="align-right" id="get-blance-due-from-ledger">@format_currency($ledger_details['all_balance_due'])</td>
			</tr>
		</table>
	</div>
</div>
	...
	<table class="table table-striped @if(!empty($for_pdf)) table-pdf td-border @endif" id="ledger_table" style="text-align: center;">
		...
		<tbody>
			@foreach($ledger_details['ledger'] as $data)
				...
				@php
					$row_amt_1 = (isset($data['debit']) && $data['debit'] != '') ? $data['debit'] : (isset($data['credit']) && $data['credit'] != '' ? $data['credit'] : '');
				@endphp
				<tr @if(!empty($for_pdf) && $loop->iteration % 2 == 0) class="odd" @endif>
					...
					<td class="ws-nowrap align-right">@if($row_amt_1 != '') @format_currency($row_amt_1) @endif</td>
					<td>
						{{$data['payment_method']}}
						@if($row_amt_1 != '')<br>@format_currency($row_amt_1)@endif
					</td>
					<td>
						{!! $data['others'] !!}
