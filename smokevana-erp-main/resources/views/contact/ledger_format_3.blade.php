...
<div class="col-md-6 col-sm-6 col-xs-6 @if(!empty($for_pdf)) width-50 f-left @endif">
	<p class="blue-heading p-4 width-50 gradiantDiv">@lang('lang_v1.to'):</p>
...
<div class="col-md-6 col-sm-6 col-xs-6 text-right align-right @if(!empty($for_pdf)) width-50 f-left @endif ">
	<h3 class="mb-0 blue-heading p-4 gradiantDiv">@lang('lang_v1.account_summary')</h3>
	...
	<table class="table @if(!empty($for_pdf)) table-pdf td-border @endif" id="ledger_table">
		<thead >
			<tr class="row-border">
				...
				<th width="10%" class="text-center">@lang('sale.amount')</th>
				<th width="10%" class="text-center">@lang('lang_v1.balance')</th>
				<th width="8%" class="text-center">@lang('lang_v1.payment_method')</th>
		...
		<tbody>
			@foreach($ledger_details['ledger'] as $data)
				@php
					$row_amt = (isset($data['debit']) && $data['debit'] != '') ? $data['debit'] : (isset($data['credit']) && $data['credit'] != '' ? $data['credit'] : '');
				@endphp
				...
				@endif>
					...
					<td class="ws-nowrap align-right">@if($row_amt != '') @format_currency($row_amt) @endif</td>
					<td class="ws-nowrap align-right">{{$data['balance']}}</td>
					<td>
						{{$data['payment_method']}}
						@if($row_amt != '')<br>@format_currency($row_amt)@endif
					</td>
					<td>
						{!! $data['others'] !!}
				...
				@if(!empty($data['transaction_type']) && $data['transaction_type'] == 'sell')
					<tr>
						<td colspan="9" class="bg-light-gray" style="padding: 0 20px 10px;">
							@include('contact.partials.sale_line_details', ['sell' => (object)$data, 'enabled_modules' => [], 'is_warranty_enabled' => false, 'for_ledger' => true])
						</td>
					</tr>
				@endif
