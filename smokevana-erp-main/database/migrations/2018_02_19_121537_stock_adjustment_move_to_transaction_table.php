...
return new class extends Migration
{
    ...
    public function down()
    {
        ...
        // Drop foreign key constraints from stock_adjustment_lines first
        if (Schema::hasTable('stock_adjustment_lines')) {
            $foreignKeys = DB::select("
                SELECT CONSTRAINT_NAME
                FROM information_schema.KEY_COLUMN_USAGE 
                WHERE TABLE_SCHEMA = DATABASE() 
                AND TABLE_NAME = 'stock_adjustment_lines'
                AND REFERENCED_TABLE_NAME IS NOT NULL
            ");
            
            foreach ($foreignKeys as $fk) {
                try {
                    DB::statement("ALTER TABLE `stock_adjustment_lines` DROP FOREIGN KEY `{$fk->CONSTRAINT_NAME}`");
                } catch (\Exception $e) {
        ...
        // Drop stock_adjustment_lines table
        Schema::dropIfExists('stock_adjustment_lines');
        
        // Drop stock_adjustments_temp table
        Schema::dropIfExists('stock_adjustments_temp');
        
        // Remove columns added to transactions table
        if (Schema::hasTable('transactions')) {
            Schema::table('transactions', function (Blueprint $table) {
                ...
                if (Schema::hasColumn('transactions', 'total_amount_recovered')) {
                    $table->dropColumn('total_amount_recovered');
                }
            });
            
            // Revert transaction type enum
            DB::statement("ALTER TABLE `transactions` CHANGE `type` `type` ENUM('purchase','sell','expense') DEFAULT NULL");
        }
